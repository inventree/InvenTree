...
- [#11500](https://github.com/inventree/InvenTree/pull/11500) changes the default database behaviour if using SQLite as the database backend. The default transaction mode is now "IMMEDIATE", which allows for better concurrency at the cost of potentially increased locking. If you are using SQLite, and wish to return to the previous behaviour, you can set the `transaction_mode` option to "DEFERRED" in the database configuration (either via environment variable or config file).
