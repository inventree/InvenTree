...
if [ "${TAG_SHA:0:8}" != "$SHA" ]; then
  ...
  curl  -L -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/$REPO/actions/runs?head_sha=$SHA > runs.json    artifact_url = $(jq -r '.workflow_runs.[] | select(.name=="QC").artifacts_url' runs.json)
