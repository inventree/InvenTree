...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Отображать данные строки');
...
function downloadBomTemplate(options={}) {
    ...
    constructFormBody({}, {
        ...
        fields: {
            format: {
                label: 'Формат',
                help_text: 'Выбрать формат файла',
                required: true,
                type: 'choice',
                value: format,
...
function loadBomTable(table, options={}) {
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Расходники)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Необязательно)</small>';
            }
    ...
    if (options.editable) {
        ...
        cols.push({
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Добавить элемент BOM' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Добавить элемент BOM
                </button>
                `;
            }
        });
