...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/?supplier_detail=true', {
        ...
        title: options.title || 'Vytvořit nákupní objednávku',
    });
}
        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Počet kusů v balení: ${formatDecimal(pack_quantity)} ${units}`;
...
function orderParts(parts_list, options={}) {
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Díl</th>
                <th style='min-width: 300px;'>Díl dodavatele</th>
                <th style='min-width: 300px;'>Purchase Order</th>
                <th style='min-width: 50px;'>Množství</th>
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Počet kusů v balení: ${pack_quantity} ${units}`;
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                Počet kusů v balení: ${pack_quantity}<br>
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Kód dávky',
                help_text: 'Zadat kód dávky pro příchozí položky skladu',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
        ...
        const sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                required: false,
                label: 'Sériová čísla',
                help_text: 'Zadat sériová čísla pro příchozí skladové položky',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th style='min-width: 150px;'>Stav</th>
                <th></th>
            </tr>
        </thead>
        <tbody>
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Počet kusů v balení: ${pack_quantity} - Total Quantity: ${total}${units}'></span>`;
                    }
