...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantityString += ` - <small>Parti: ${item.batch}</small>`;
        }
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Brukt av produksjonsordre';
    ...
    let actions = [
        {
            ...
            title: 'Legg til lagerbeholdning',
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (details.buildorder) {
                html += `<tr><th>Produksjonsordre</th>`;
                html += `<td>`;
