...
function yesNoLabel(value, options={}) {
    ...
    if (toBool(value)) {
        text = options.pass || 'SIM';
        color = 'bg-success';
    } else {
        text = options.fail || 'NÃO';
        color = 'bg-warning';
    }
...
function renderClipboard(s, prepend=false) {
    ...
    let clipString = `<span class="d-none d-xl-inline"><button class="btn clip-btn" type="button" data-bs-toggle='tooltip' title='copiar para área de transferência'><em class="fas fa-copy"></em></button></span>`;
...
async function loadApiIconPacks() {
    if(!window._ICON_PACKS) {
        const packs = await inventreeGet('/api/icons/');
