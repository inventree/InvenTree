    ...
    <div class='form-group'>
        ...
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Barcode mit angeschlossener Webcam scannen' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Optionale Bemerkung zu Bestandsübertragung eingeben';
    var placeholder = options.placeholder || 'Eine Bemerkung anführen';
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Ungültige Serverantwort<br>Status: '${status}'`
    );
}
...
function unlinkBarcode(data, options={}) {
    ...
    showQuestionDialog(
        ...
        html,
        {
            accept_text: 'Verknüpfung aufheben',
            accept: function() {
                inventreePut(
                    '/api/barcode/unlink/',
                    data,
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    barcodeDialog(
        'Artikel per Barcode-Scan zu Lagerort hinzufügen',
        {
            details: 'Barcode des Lagerartikels scannen um ihn an diesen Ort hinzuzufügen',
            headerContent: table,
            preShow: function() {
                            ...
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    ...
                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Lagerartikel bereits an diesem Standort vorhanden');
                                        return;
                                    }
...
function barcodeCheckInStockLocations(location_id, options={}) {
    ...
    barcodeDialog(
        'Lagerbehälter an diesen Ort einscannen',
        {
            details: 'Barcode des Lagerbehälters scannen um ihn an diesen Ort hinzuzufügen',
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, 'Barcode entspricht keinem gültigen Lagerort', 'warning');
                }
            }
        }
    );
