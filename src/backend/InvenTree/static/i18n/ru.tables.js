...
function constructExpandCollapseButtons(table, idx=0) {
    ...
    return [
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Развернуть все строки'><span class='fas fa-expand'></span></button>`,
            event: function() {
...
(function($) {
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatRecordsPerPage: function(pageNumber) {
            return `${pageNumber} строк на страницу`;
        },
        formatShowingRows: function(pageFrom, pageTo, totalRows) {

            if (totalRows === undefined || isNaN(totalRows)) {
                return 'Отображение всех строк';
            } else {
                return `Отображается ${pageFrom} в ${pageTo} of ${totalRows} строки`;
            }
        },
