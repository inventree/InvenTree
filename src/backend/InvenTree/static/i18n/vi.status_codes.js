



/* globals
*/

/* exported
    buildStatusDisplay,
    purchaseOrderStatusDisplay,
    returnOrderStatusDisplay,
    returnOrderLineItemStatusDisplay,
    salesOrderStatusDisplay,
    stockHistoryStatusDisplay,
    stockStatusDisplay,
*/


/*
 * Generic function to render a status label
 */
function renderStatusLabel(key, codes, options={}) {

    let text = null;
    let label = null;

    // Find the entry which matches the provided key
    for (var name in codes) {
        let entry = codes[name];

        if (entry.key == key) {
            text = entry.value;
            label = entry.label;
            break;
        }
    }

    if (!text) {
        console.error(`renderStatusLabel could not find match for code ${key}`);
    }

    // Fallback for color
    label = label || 'bg-dark';

    if (!text) {
        text = key;
    }

    let classes = `badge rounded-pill ${label}`;

    if (options.classes) {
        classes += ` ${options.classes}`;
    }

    return `<span class='${classes}'>${text}</span>`;
}



/*
 * Status codes for the stock model.
 * Generated from the values specified in "status_codes.py"
 */
const stockCodes = {
    
    'OK': {
        key: 10,
        value: 'Đồng ý',
        label: 'bg-success',
    },
    
    'ATTENTION': {
        key: 50,
        value: 'Chú ý cần thiết',
        label: 'bg-warning',
    },
    
    'DAMAGED': {
        key: 55,
        value: 'Bị hư hại',
        label: 'bg-warning',
    },
    
    'DESTROYED': {
        key: 60,
        value: 'Đã hủy',
        label: 'bg-danger',
    },
    
    'REJECTED': {
        key: 65,
        value: 'Đã từ chối',
        label: 'bg-danger',
    },
    
    'LOST': {
        key: 70,
        value: 'Mất',
        label: 'bg-dark',
    },
    
    'QUARANTINED': {
        key: 75,
        value: 'Đã cách ly',
        label: 'bg-info',
    },
    
    'RETURNED': {
        key: 85,
        value: 'Đã trả lại',
        label: 'bg-warning',
    },
    
};

/*
 * Render the status for a stock object.
 * Uses the values specified in "status_codes.py"
 */
function stockStatusDisplay(key, options={}) {
    return renderStatusLabel(key, stockCodes, options);
}



/*
 * Status codes for the stockHistory model.
 * Generated from the values specified in "status_codes.py"
 */
const stockHistoryCodes = {
    
    'LEGACY': {
        key: 0,
        value: 'Mục theo dõi kho cổ điển',
        label: 'bg-secondary',
    },
    
    'CREATED': {
        key: 1,
        value: 'Kho hàng đã được khởi tạo',
        label: 'bg-secondary',
    },
    
    'EDITED': {
        key: 5,
        value: 'Sửa kho hàng',
        label: 'bg-secondary',
    },
    
    'ASSIGNED_SERIAL': {
        key: 6,
        value: 'Số sê ri đã được gán',
        label: 'bg-secondary',
    },
    
    'STOCK_COUNT': {
        key: 10,
        value: 'Kho đã đếm',
        label: 'bg-secondary',
    },
    
    'STOCK_ADD': {
        key: 11,
        value: 'Kho được thêm thủ công',
        label: 'bg-secondary',
    },
    
    'STOCK_REMOVE': {
        key: 12,
        value: 'Kho được xóa thủ công',
        label: 'bg-secondary',
    },
    
    'STOCK_MOVE': {
        key: 20,
        value: 'Vị trí đã thay đổi',
        label: 'bg-secondary',
    },
    
    'STOCK_UPDATE': {
        key: 25,
        value: 'Kho hàng đã được cập nhật',
        label: 'bg-secondary',
    },
    
    'INSTALLED_INTO_ASSEMBLY': {
        key: 30,
        value: 'Đã cài đặt vào bộ phận lắp ráp',
        label: 'bg-secondary',
    },
    
    'REMOVED_FROM_ASSEMBLY': {
        key: 31,
        value: 'Di rời khỏi bộ phận lắp ráp',
        label: 'bg-secondary',
    },
    
    'INSTALLED_CHILD_ITEM': {
        key: 35,
        value: 'Mục thành phần đã cài đặt',
        label: 'bg-secondary',
    },
    
    'REMOVED_CHILD_ITEM': {
        key: 36,
        value: 'Mục thành phần đã di rời',
        label: 'bg-secondary',
    },
    
    'SPLIT_FROM_PARENT': {
        key: 40,
        value: 'Tách từ mục cha',
        label: 'bg-secondary',
    },
    
    'SPLIT_CHILD_ITEM': {
        key: 42,
        value: 'Tách mục con',
        label: 'bg-secondary',
    },
    
    'MERGED_STOCK_ITEMS': {
        key: 45,
        value: 'Kho hàng đã được gộp',
        label: 'bg-secondary',
    },
    
    'CONVERTED_TO_VARIANT': {
        key: 48,
        value: 'Đã chuyển đổi sang biến thể',
        label: 'bg-secondary',
    },
    
    'BUILD_OUTPUT_CREATED': {
        key: 50,
        value: 'Đầu ra đơn đặt bản dựng đã được tạo',
        label: 'bg-secondary',
    },
    
    'BUILD_OUTPUT_COMPLETED': {
        key: 55,
        value: 'Đầu ra đơn đặt bản dựng đã hoàn thành',
        label: 'bg-secondary',
    },
    
    'BUILD_OUTPUT_REJECTED': {
        key: 56,
        value: 'Đầu ra đơn đặt bản dựng bị từ chối',
        label: 'bg-secondary',
    },
    
    'BUILD_CONSUMED': {
        key: 57,
        value: 'Tiêu hao bởi đơn đặt bản dựng',
        label: 'bg-secondary',
    },
    
    'SHIPPED_AGAINST_SALES_ORDER': {
        key: 60,
        value: 'Vận chyển dựa vào đơn đặt bản dựng',
        label: 'bg-secondary',
    },
    
    'RECEIVED_AGAINST_PURCHASE_ORDER': {
        key: 70,
        value: 'Đã nhận dựa vào đơn đặt hàng mua',
        label: 'bg-secondary',
    },
    
    'RETURNED_AGAINST_RETURN_ORDER': {
        key: 80,
        value: 'Trả hành dựa vào đơn hàng trả lại',
        label: 'bg-secondary',
    },
    
    'SENT_TO_CUSTOMER': {
        key: 100,
        value: 'Gửi đến khách hàng',
        label: 'bg-secondary',
    },
    
    'RETURNED_FROM_CUSTOMER': {
        key: 105,
        value: 'Bị trả lại từ khách hàng',
        label: 'bg-secondary',
    },
    
};

/*
 * Render the status for a stockHistory object.
 * Uses the values specified in "status_codes.py"
 */
function stockHistoryStatusDisplay(key, options={}) {
    return renderStatusLabel(key, stockHistoryCodes, options);
}



/*
 * Status codes for the build model.
 * Generated from the values specified in "status_codes.py"
 */
const buildCodes = {
    
    'PENDING': {
        key: 10,
        value: 'Đợi duyệt',
        label: 'bg-secondary',
    },
    
    'PRODUCTION': {
        key: 20,
        value: 'Sản xuất',
        label: 'bg-primary',
    },
    
    'ON_HOLD': {
        key: 25,
        value: 'On Hold',
        label: 'bg-warning',
    },
    
    'CANCELLED': {
        key: 30,
        value: 'Đã hủy',
        label: 'bg-danger',
    },
    
    'COMPLETE': {
        key: 40,
        value: 'Hoàn thành',
        label: 'bg-success',
    },
    
};

/*
 * Render the status for a build object.
 * Uses the values specified in "status_codes.py"
 */
function buildStatusDisplay(key, options={}) {
    return renderStatusLabel(key, buildCodes, options);
}



/*
 * Status codes for the purchaseOrder model.
 * Generated from the values specified in "status_codes.py"
 */
const purchaseOrderCodes = {
    
    'PENDING': {
        key: 10,
        value: 'Đợi duyệt',
        label: 'bg-secondary',
    },
    
    'PLACED': {
        key: 20,
        value: 'Đã đặt',
        label: 'bg-primary',
    },
    
    'ON_HOLD': {
        key: 25,
        value: 'On Hold',
        label: 'bg-warning',
    },
    
    'COMPLETE': {
        key: 30,
        value: 'Hoàn thành',
        label: 'bg-success',
    },
    
    'CANCELLED': {
        key: 40,
        value: 'Đã hủy',
        label: 'bg-danger',
    },
    
    'LOST': {
        key: 50,
        value: 'Mất',
        label: 'bg-warning',
    },
    
    'RETURNED': {
        key: 60,
        value: 'Đã trả lại',
        label: 'bg-warning',
    },
    
};

/*
 * Render the status for a purchaseOrder object.
 * Uses the values specified in "status_codes.py"
 */
function purchaseOrderStatusDisplay(key, options={}) {
    return renderStatusLabel(key, purchaseOrderCodes, options);
}



/*
 * Status codes for the salesOrder model.
 * Generated from the values specified in "status_codes.py"
 */
const salesOrderCodes = {
    
    'PENDING': {
        key: 10,
        value: 'Đợi duyệt',
        label: 'bg-secondary',
    },
    
    'IN_PROGRESS': {
        key: 15,
        value: 'Đang tiến hành',
        label: 'bg-primary',
    },
    
    'SHIPPED': {
        key: 20,
        value: 'Đã chuyển',
        label: 'bg-success',
    },
    
    'ON_HOLD': {
        key: 25,
        value: 'On Hold',
        label: 'bg-warning',
    },
    
    'COMPLETE': {
        key: 30,
        value: 'Hoàn thành',
        label: 'bg-success',
    },
    
    'CANCELLED': {
        key: 40,
        value: 'Đã hủy',
        label: 'bg-danger',
    },
    
    'LOST': {
        key: 50,
        value: 'Mất',
        label: 'bg-warning',
    },
    
    'RETURNED': {
        key: 60,
        value: 'Đã trả lại',
        label: 'bg-warning',
    },
    
};

/*
 * Render the status for a salesOrder object.
 * Uses the values specified in "status_codes.py"
 */
function salesOrderStatusDisplay(key, options={}) {
    return renderStatusLabel(key, salesOrderCodes, options);
}



/*
 * Status codes for the returnOrder model.
 * Generated from the values specified in "status_codes.py"
 */
const returnOrderCodes = {
    
    'PENDING': {
        key: 10,
        value: 'Đợi duyệt',
        label: 'bg-secondary',
    },
    
    'IN_PROGRESS': {
        key: 20,
        value: 'Đang tiến hành',
        label: 'bg-primary',
    },
    
    'ON_HOLD': {
        key: 25,
        value: 'On Hold',
        label: 'bg-warning',
    },
    
    'COMPLETE': {
        key: 30,
        value: 'Hoàn thành',
        label: 'bg-success',
    },
    
    'CANCELLED': {
        key: 40,
        value: 'Đã hủy',
        label: 'bg-danger',
    },
    
};

/*
 * Render the status for a returnOrder object.
 * Uses the values specified in "status_codes.py"
 */
function returnOrderStatusDisplay(key, options={}) {
    return renderStatusLabel(key, returnOrderCodes, options);
}



/*
 * Status codes for the returnOrderLineItem model.
 * Generated from the values specified in "status_codes.py"
 */
const returnOrderLineItemCodes = {
    
    'PENDING': {
        key: 10,
        value: 'Đợi duyệt',
        label: 'bg-secondary',
    },
    
    'RETURN': {
        key: 20,
        value: 'Trả lại',
        label: 'bg-success',
    },
    
    'REPAIR': {
        key: 30,
        value: 'Sửa chữa',
        label: 'bg-primary',
    },
    
    'REPLACE': {
        key: 40,
        value: 'Thay thế',
        label: 'bg-warning',
    },
    
    'REFUND': {
        key: 50,
        value: 'Hoàn tiền',
        label: 'bg-info',
    },
    
    'REJECT': {
        key: 60,
        value: 'Từ chối',
        label: 'bg-danger',
    },
    
};

/*
 * Render the status for a returnOrderLineItem object.
 * Uses the values specified in "status_codes.py"
 */
function returnOrderLineItemStatusDisplay(key, options={}) {
    return renderStatusLabel(key, returnOrderLineItemCodes, options);
}

