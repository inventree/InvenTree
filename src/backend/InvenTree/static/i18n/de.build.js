            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.has_allocated_stock) {
                    ...
                    <div class='alert alert-block alert-warning'>
                        Lagerartikel wurden zu diesem Bauauftrag hinzugefügt
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Es sind noch unvollständige Artikel für diesen Bauauftrag vorhanden
                    </div>`;
                }
            ...
            success: function(build) {
                ...
                if (trackable) {
                    ...
                    <div class='alert alert-block alert-info'>
                        ...
                        Seriennummern für mehrere einzelne Endprodukte angeben
                    </div>
                    `;
                }
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Bestand von Endprodukt entfernen',
        );
    }
...
function deallocateStock(build_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-warning'>
    Sind Sie sicher, dass sie alle Lagerartikel von diesem Bauauftrag entfernen möchten?
    </dvi>
    `;
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-success'>
    Ausgewählte Endprodukte werden als vollständig markiert
    </div>
...
function scrapBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Ausgewählte Endprodukte werden als Ausschuss markiert
    <ul>
        <li>Ausschuss wird als verworfen markiert</li>
        <li>Zugewiesene Lagerbestände werden nicht mehr verfügbar sein</li>
        <li>Der Fertigstellungsstatus des Bauauftrags wird nicht angepasst</li>
    </ul>
    </div>
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Ausgewählte Endprodukte werden gelöscht
    <ul>
    <li>Endprodukte werden dauerhaft gelöscht</li>
    <li>Zugewiesene Lagerartikel werden in den Bestand zurückgeführt</li>
    </ul>
    </div>
    <table class='table table-striped table-condensed' id='build-complete-table'>
        <thead>
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    setupFilterList('builditems', $(table), options.filterTarget || '#filter-list-incompletebuilditems', {
        ...
        singular_name: 'Endprodukt',
        plural_name: 'Endprodukte',
        custom_actions: [{
            label: 'buildoutput',
            icon: 'fa-tools',
            title: 'Endprodukt-Aktionen',
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Teile auswählen',
            'Sie müssen mindestens einen Teil für die Zuweisung auswählen',
        );
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'Keine passenden Lagerartikel';
                        }
                    },
                    null,
                    options,
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    <ul>
        ...
        <li>Wenn ein Ersatzlager zugelassen ist, wird dieses verwendet, wenn das Primärteil nicht vorrätig ist</li>
    </ul>
    </div>
    `;
...
function loadBuildTable(table, options) {
    ...
    $(table).inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Keine Bauaufträge zur Suchanfrage';
        },
        url: '/api/build/',
        queryParams: filters,
        groupBy: false,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.responsible_detail.label == 'gruppieren') {
...
function loadBuildLineTable(table, build_id, options={}) {
    ...
    if (!options.data) {
        setupFilterList('buildlines', $(table), filterTarget, {
            ...
            singular_name: 'Bauauftragsposition',
            plural_name: 'Bauauftragspositionen',
        });
    }
    ...
    let table_options = {
        ...
        formatNoMatches: function() {
            return 'Keine Bauauftragspositionen gefunden';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Alternatives Lager erlaubt');
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        ...
                        if (available < (row.quantity - row.allocated)) {
                            ...
                            icons += makeIconBadge('fa-check-circle icon-green', 'Ausreichender Bestand vorhanden');
                        }
                        ...
                        if (available <= 0) {
                            ...
                            if ((row.available_substitute_stock > 0) && (row.available_variant_stock > 0)) {
                                extra = 'Alternatives Lager und Ersatzteillager einschließen';
                            } else if (row.available_variant_stock > 0) {
                                extra = 'Alternatives Lager einschließen';
                            } else if (row.available_substitute_stock > 0) {
                                extra = 'Ersatzteillager einschließen';
                            }
                    ...
                    if (row.in_production && row.in_production > 0) {
                        icons += makeIconBadge('fa-tools icon-blue', `In Produktion: ${formatDecimal(row.in_production)}`);
                    }

                    if (row.external_stock > 0) {
                        icons += makeIconBadge('fa-sitemap', `Externes Lager: ${row.external_stock}`);
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part_detail.trackable && !options.output) {
                        ...
                        return `
                        <div>
                            <em>Verfolgtes Objekt</em>
                            <span title='Zuweisung von nachverfolgbaren Artikeln zu einzelnen Bauprodukten' class='fas fa-info-circle icon-blue' />
                        </div>`;
                    }
                    ...
                    if (row.allocated > 0) {
                        buttons += makeRemoveButton('button-unallocate', pk, 'Bestands-Zuordnung löschen');
                    }
