...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>يلغي</button>`;
    }

    // Add in a "submit" button
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>إرسال</button>`;
    }
    ...
    modalSetSubmitText(modal_name, options.submitText || 'إرسال');
    modalSetCloseText(modal_name, options.closeText || 'يلغي');
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.closeText = options.cancel_text || 'يلغي';
...
function openModal(options) {
    ...
    var submit_text = options.submit_text || 'إرسال';
    var close_text = options.close_text || 'إغلاق';
