...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        if (!grouped && row.result != null) {
            html += makeEditButton('button-test-edit', row.testId, 'Editar resultado de prueba');
            html += makeDeleteButton('button-test-delete', row.testId, 'Eliminar resultado de prueba');
        }
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Eliminar resultado de prueba</strong>
        </div>`;
