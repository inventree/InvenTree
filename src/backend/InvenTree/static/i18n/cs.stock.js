...
function createNewStockItem(options={}) {
    ...
    options.title = 'Nová skladová položka';
    options.method = 'POST';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                var details = `
                <br>Množství: ${response.quantity}
                <br>Sériová čísla: ${response.serial_numbers}
                `;
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            ...
            quantity = `Množství: ${item.quantity}`;
        }
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantityString += ` - <small>Šarže: ${item.batch}</small>`;
        }
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (details.salesorder) {
                html += `<tr><th>Prodejní objednávka</th>`;
                html += '<td>';
            ...
            if (details.stockitem) {
                html += '<tr><th>Skladové položky</td>';
