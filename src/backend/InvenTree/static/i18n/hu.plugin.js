...
function loadPluginTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nem találhatók pluginok';
        },
        columns: [
            {
                field: 'name',
                ...
                formatter: function(value, row) {
                    ...
                    if (!row.is_installed) {
                        html += `<span class='fa fa-question-circle' title='Ez a plugin már nincs telepítve'></span>`;
                    } else if (row.active) {
                        html += `<span class='fa fa-check-circle icon-green' title='Ez a plugin aktív'></span>`;
                    } else {
                        html += `<span class='fa fa-times-circle icon-red' title ='Ez a plugin telepítve van ugyan, de nem aktív'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.custom && !row.is_builtin && row.is_installed) {
                        if (row.active) {
                            buttons += makeIconButton('fa-stop-circle icon-red', 'btn-plugin-disable', row.key, 'Plugin kikapcsolása');
                        } else {
                            buttons += makeIconButton('fa-play-circle icon-green', 'btn-plugin-enable', row.key, 'Plugin bekapcsolása');
                        }
                    }
...
function activatePlugin(plugin_id, active=true) {
    ...
    Biztosan bekapcsolod ezt a plugint?
    </span>
    ` : `
    <span class='alert alert-block alert-danger'>
    Biztosan kikapcsolod ezt a plugint?
    </span>
    `;

    constructForm(null, {
        title: active ? 'Plugin bekapcsolása' : 'Plugin kikapcsolása',
        preFormContent: html,
        confirm: true,
        submitText: active ? 'Bekapcsolás' : 'Letiltás',
            ...
            inventreePut(
                ...
                },
                {
                    ...
                    success: function() {
                        ...
                        addCachedAlert('Plugin frissítve', {style: 'success'});
                        location.reload();
                    },
                    error: function(xhr) {
                        $(opts.modal).modal('hide');
