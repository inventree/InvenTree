        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    if (enableDelete) {
                        bDel = `<button title='Удалить уведомление' class='notification-delete btn btn-outline-secondary' type='button' pk='${row.pk}'><span class='fas fa-trash-alt icon-red'></span></button>`;
                    }
...
function getReadEditButton(pk, state, small=false) {
    ...
    if (state) {
        bReadText = 'Пометить как непрочитанное';
        bReadIcon = 'fas fa-bookmark icon-red';
        bReadTarget = 'unread';
    } else {
        bReadText = 'Пометить как прочитанное';
