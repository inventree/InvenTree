...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>odustati</button>`;
    }

    // Add in a "submit" button
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>Пошаљи</button>`;
    }
    ...
    modalSetSubmitText(modal_name, options.submitText || 'Пошаљи');
    modalSetCloseText(modal_name, options.closeText || 'odustati');
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.closeText = options.cancel_text || 'odustati';
...
function openModal(options) {
    ...
    var submit_text = options.submit_text || 'Пошаљи';
    var close_text = options.close_text || 'Затвори';
