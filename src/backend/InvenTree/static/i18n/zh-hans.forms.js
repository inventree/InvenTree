...
function insertPersistButton(options) {
    ...
    var message = options.persistMessage || '保持此表单打开';
...
function initializeRelatedField(field, fields, options={}) {
    ...
    if(field.tree_picker) {
        ...
        button.on("click", () => {
            ...
            const title = '选择' + " " + options.actions[name].label;
            const content = `
                <div class="mb-1">
                    <div class="input-group mb-2">
                        <input class="form-control" type="text" id="${name}_tree_search" placeholder="搜索 ${options.actions[name].label}..." />
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <button class='input-group-text form-clear' id='clear_${field_name}' title='清空输入'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>文件列</th><th>字段名称</th></tr>`;
