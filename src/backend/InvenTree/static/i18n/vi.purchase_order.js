...
function createPurchaseOrder(options={}) {
    ...
    constructForm('/api/order/po/?supplier_detail=true', {
        ...
        title: options.title || 'Tạo đơn mua hàng',
    });
}
        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Số lượng gói: ${formatDecimal(pack_quantity)} ${units}`;
...
function orderParts(parts_list, options={}) {
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>Nguyên liệu</th>
                <th style='min-width: 300px;'>Sản phẩm nhà cung cấp</th>
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Số lượng gói: ${pack_quantity} ${units}`;
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        var batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Mã lô hàng',
                help_text: 'Nhập mã lô cho hàng trong kho đang đến',
                icon: 'fa-layer-group',
            },
            {
                hideLabels: true,
        ...
        const sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                required: false,
                label: 'Số sê-ri',
                help_text: 'Nhập số sê ri cho hàng trong kho đang đến',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
                ...
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Scan Item Barcode', {
                        ...
                        onScan: function(response, barcode_options) {
                            ...
                            showBarcodeMessage(barcode_options.modal, 'Mã vạch phù hợp với hàng hóa hiện có');
                        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Số lượng gói: ${pack_quantity} - Tổng số lượng: ${total}${units}'></span>`;
                    }
