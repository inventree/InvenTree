...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || '使用条形码扫描仪在此处扫描条形码数据';
    ...
    <div class='form-group'>
        ...
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='使用连接的网络摄像头扫描条形码' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || '输入库存转移的可选注释';
    var placeholder = options.placeholder || '添加备注';
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || '扫描条形码数据';
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, '移除库存项')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        '将库存商品扫描到位置',
        {
            details: '扫描库存商品条形码以登记到此位置',
            headerContent: table,
            preShow: function() {
                modalSetSubmitText(modal, '登记');
                modalEnable(modal, false);
                reloadTable();
            },
            onShow: function() {
            ...
            onSubmit: function() {
                ...
                if (entries.length == 0) {
                    showBarcodeMessage(modal, '未提供条形码', 'warning');
                    return;
                }
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, '库存项已扫描', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, '库存项已在此位置');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, '已添加库存项', 'success');
                    ...
                    showBarcodeMessage(modal, '条形码与有效库存项目不匹配', 'warning');
                }
            },
        }
    );
        ...
        '将库存集装箱扫描到指定位置',
        {
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, '条形码与有效库存位置不匹配', 'warning');
                }
            }
        }
    );
