...
function categoryFields(options={}) {
    let fields = {
        parent: {
            label: 'Parent',
            help_text: 'Categoría principal de parte',
            required: false,
            tree_picker: {
                url: '/api/part/category/tree/',
            },
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        ...
        follow: true,
        persist: true,
        persistMessage: 'Crear nueva categoría después de esta',
        successMessage: 'Categoría de parte creada'
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'Mover a la categoría principal',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        ...
        fields: {
            'delete_parts': {
                label: 'Acción para partes en esta categoría',
                choices: subChoices,
                type: 'choice'
            },
            'delete_child_categories': {
                label: 'Acción para categorías secundarias',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        title: 'Crear Parte',
        persist: true,
        persistMessage: 'Crear otra parte después de esta',
        successMessage: 'Parte creada exitosamente',
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            elements.push(`No hay stock disponible`);
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Cantidad de paquete: ${pq} - Cantidad Total: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Cantidad de paquete: ${pq} - Cantidad Total: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.results && row.results > 0) {
                        html += `
                        <span class='badge bg-dark rounded-pill float-right' title='${row.results} resultados'>
                            ${row.results}
                        </span>`;
                    }
...
function loadPartSchedulingChart(canvas_id, part_id) {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        ...
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', 'Fecha especificada es en el pasado');
                    }
    ...
    if (n_entries < 1) {
        ...
        var message = `
        <div class='alert alert-block alert-info'>
            No hay información de planificación disponible para esta parte.
        </div>`;
    ...
    var data = {
        datasets: [
            {
                label: 'Cantidad de Stock Programadas',
