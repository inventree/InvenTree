...
function partGroups() {
    ...
    return {
        ...
        initial_supplier: {
            title: 'Исходные данные о поставщике',
            collapsible: true,
...
function editPart(pk) {
    ...
    constructForm(url, {
        ...
        reload: true,
        successMessage: 'Деталь изменена',
    });
}
...
function deletePart(pk, options={}) {
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            ...
            <ul>
            <li>Любые складские позиции для этой запчасти будут удалены</li>
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`Низкий запас: ${part.total_in_stock}`);
            ...
            if (options.hideTotalStock) {
                elements.push(`Доступно: ${part.unallocated_stock}`);
            } else {
                elements.push(`Доступно: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Нет запасов`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`В заказе: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Производится: ${part.building}`);
    }
    ...
    if (options.showDemandInfo) {
        ...
        if (demand) {
            elements.push(`Требуется: ${demand}`);
        }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Ед. Изм.: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Виртуальная Деталь');
    }
    ...
    if (part.assembly) {
        html += makeIconBadge('fa-tools', 'Производимая Деталь');
    }

    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Деталь с подпиской');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Продаваемая деталь');
    }

    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Неактивный</span> `;
    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Кол-во в упаковке: ${pq} - Общее количество: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Кол-во в упаковке: ${pq} - Общее количество: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Получено</span>`;
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Нет запасов</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Нет запасов</span>`;
    }

    rows += `<tr><td><b>Склад</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>В заказе</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Производится</b></td><td>${part.building} ${units}</td></tr>`;
    }
...
function loadPartTable(table, url, options={}) {
    ...
    columns.push({
        ...
        title: 'Место хранения по умолчанию',
        sortable: true,
        formatter: function(value, row) {
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Категория с подпиской');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.results && row.results > 0) {
                        html += `
                        <span class='badge bg-dark rounded-pill float-right' title='${row.results} результаты'>
                            ${row.results}
                        </span>`;
                    }
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', 'Приблизительный');
                    }
