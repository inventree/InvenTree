...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        ...
        if (labels_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-labels-${tableKey}'><span class='fas fa-tag'></span> Etiketten drucken</a></li>`;
        }

        if (report_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-report-${tableKey}'><span class='fas fa-file-pdf'></span> Berichte drucken</a></li>`;
        }
    ...
    element.find(`#${add}`).click(function clicked() {
        ...
        if (!addClicked) {
            ...
            html += `<button title='Filter erstellen' class='btn btn-outline-secondary filter-button' id='${make}'><span class='fas fa-plus'></span></button>`;
            html += `</div>`;
...
function getFilterOptionValue(tableKey, filterKey, valueKey) {
    ...
    if (filter.type == 'bool') {
        if (value == '1') return 'wahr';
        if (value == '0') return 'falsch';
