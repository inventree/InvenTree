        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Đơn đặt bản dựng có đầu ra chưa hoàn thiện</strong><br>
                ...
                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Kho được yêu cầu chưa được phân bổ hết không gian</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Số lượng bản dựng được yêu cầu chưa được hoàn thành</div>`;
                }
            }
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Số sê-ri: ${output.serial}`;
    } else {
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Xóa hàng');
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Hàng loạt: ${row.batch})</small>`;
                    }
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Nguyên liệu</th>
                <th>Đã cấp phát</th>
                <th style='min-width: 250px;'>Kho hàng</th>
                <th>Số lượng</th>
                <th></th>
            </tr>
        </thead>
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Không có vị trí nào được đặt</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        ...
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Kho không đủ hạn mức khả dụng');
                        } else {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `Bật đơn hàng: ${formatDecimal(row.on_order)}`);
                    }

                    if (row.in_production && row.in_production > 0) {
                        icons += makeIconBadge('fa-tools icon-blue', `Đang sản xuất: ${formatDecimal(row.in_production)}`);
                    }
