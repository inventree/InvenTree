...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Няма наличност</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Няма наличност</span>`;
    }

    rows += `<tr><td><b>Наличност</b></td><td>${stock}</td></tr>`;
