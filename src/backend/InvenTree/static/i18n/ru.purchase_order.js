        ...
        part: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                inventreeGet(`/api/company/part/${value}/`,
                    ...
                    if (pack_quantity != 1) {
                        var txt = `<span class='fas fa-info-circle icon-blue'></span> Кол-во в упаковке: ${formatDecimal(pack_quantity)} ${units}`;
                ...
                var onSupplierPartChanged = function(value, name, field, opts) {
                    ...
                    if (value != null) {
                        inventreeGet(
                            ...
                            if (pack_quantity != 1) {
                                var txt = `<span class='fas fa-info-circle icon-blue'></span> Кол-во в упаковке: ${pack_quantity} ${units}`;
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                Кол-во в упаковке: ${pack_quantity}<br>
        ...
        const sn_input = constructField(
            `items_serial_numbers_${pk}`,
            {
                ...
                help_text: 'Введите серийные номера для входящих складских позиций',
                icon: 'fa-hashtag',
            },
            {
                hideLabels: true,
        ...
        if (global_settings.BARCODE_ENABLE) {
            buttons += makeIconButton('fa-qrcode', 'button-row-add-barcode', pk, 'Добавить штрихкод');
            buttons += makeIconButton('fa-unlink icon-red', 'button-row-remove-barcode', pk, 'Удалить штрихкод', {hidden: true});
        }

        buttons += makeIconButton('fa-sitemap', 'button-row-add-location', pk, 'Выберите место хранения', {
            ...
            if (global_settings.BARCODE_ENABLE) {
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Scan Item Barcode', {
                        details: 'Сканировать штрихкод входящего элемента (не должен совпадать с любой существующей складской позицией)',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Кол-во в упаковке: ${pack_quantity} - Общее количество: ${total}${units}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_edit) {
                        ...
                        buttons += makeEditButton('button-line-edit', pk, 'Редактировать Позицию');
                        buttons += makeDeleteButton('button-line-delete', pk, 'Удалить позицию');
                    }
