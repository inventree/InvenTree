...
function serializeStockItem(pk, options={}) {
    ...
    if (options.part) {
        ...
        inventreeGet(`/api/part/${options.part}/serial-numbers/`, {}, {
            success: function(data) {
                if (data.next) {
                    options.fields.serial_numbers.placeholder = `Próximo número de série disponível: ${data.next}`;
                } else if (data.latest) {
                    options.fields.serial_numbers.placeholder = `Último número de série: ${data.latest}`;
                }
            },
            async: false,
        });
    ...
    options.confirmMessage = 'Confirmar Serialização de Estoque';
...
function stockLocationFields(options={}) {
    var fields = {
        ...
        location_type: {
            secondary: {
                title: 'Adicionar Tipo de Localização',
                fields: function() {
...
function editStockLocation(pk, options={}) {
    ...
    options.title = 'Editar Local de Estoque';
...
function createStockLocation(options={}) {
    ...
    options.method = 'POST';
    options.fields = stockLocationFields(options);
    options.title = 'Novo Local de Estoque';
...
function stockItemFields(options={}) {
    var fields = {
                    ...
                    if (data.trackable) {
                        ...
                        inventreeGet(`/api/part/${data.pk}/serial-numbers/`, {}, {
                            success: function(data) {
                                var placeholder = '';
                                if (data.next) {
                                    placeholder = `Próximo número de série disponível: ${data.next}`;
                                } else if (data.latest) {
                                    placeholder = `Último número de série: ${data.latest}`;
                                }
        ...
        quantity: {
            help_text: 'Inserir quantidade inicial deste item de estoque',
        },
        serial_numbers: {
            icon: 'fa-hashtag',
            type: 'string',
            label: 'Números de Série',
            help_text: 'Insira os números de série para novo estoque (ou deixe em branco)',
            required: false,
        },
        serial: {
            icon: 'fa-hashtag',
...
function duplicateStockItem(pk, options) {
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            showAlertOrCache('Item de estoque duplicado', true, {style: 'success'});
    ...
    inventreeGet(`/api/stock/${pk}/`, {}, {
        success: function(data) {
            ...
            options.method = 'POST';
            options.title = 'Duplicar Item de Estoque';
...
function deleteStockItem(pk, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Você tem certeza que deseja excluir este item de estoque?
    </div>`;
...
function editStockItem(pk, options={}) {
    ...
    options.title = 'Editar Item do Estoque';
...
function adjustStock(action, items, options={}) {
    ...
    case 'move':
        ...
        break;
    case 'count':
        formTitle = 'Contar Estoque';
        actionTitle = 'Contar';
        url = '/api/stock/count/';
        break;
    case 'take':
        formTitle = 'Remover Estoque';
        actionTitle = 'Pegar';
        url = '/api/stock/remove/';
        break;
    case 'add':
        formTitle = 'Adicionar Estoque';
        actionTitle = 'Adicionar';
        url = '/api/stock/add/';
        break;
    case 'delete':
        formTitle = 'Excluir Estoque';
        allowSerializedStock = true;
        break;
    default:
        break;
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantityString += ` - <small>Lote: ${item.batch}</small>`;
        }
        ...
        if (actionTitle != null) {
            actionInput = constructField(
                `items_quantity_${pk}`,
                {
                    ...
                    max_value: maxValue,
                    value: value,
                    title: readonly ? 'Quantidade não pode ser ajustada para estoque serializado' : 'Especifique quantidade no estoque',
                    required: true,
                },
                {
                    hideLabels: true,
    ...
    if (itemCount == 0) {
        showAlertDialog(
            'Selecionar Itens de Estoque',
            'Selecione ao menos um item de estoque disponível',
        );
...
function passFailBadge(result) {
    ...
    if (result) {
        return `<span class='badge badge-right rounded-pill bg-success'>PASSOU</span>`;
    } else {
        return `<span class='badge badge-right rounded-pill bg-danger'>FALHOU</span>`;
    }
}

function noResultBadge() {
    return `<span class='badge badge-right rounded-pill bg-info'>SEM RESULTADO</span>`;
}
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        if (row.requires_attachment == false && row.requires_value == false && !row.result) {
            ...
            html += makeIconButton('fa-check-circle icon-green', 'button-test-tick', row.test_name, 'Passou no teste');
        }

        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.templateId, 'Adicionar resultado de teste');

        if (!grouped && row.result != null) {
            html += makeEditButton('button-test-edit', row.testId, 'Editar resultados de teste');
            html += makeDeleteButton('button-test-delete', row.testId, 'Excluir resultado do teste');
        }
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhum resultado de teste encontrado';
        },
        queryParams: filters,
        original: params,
        onPostBody: function() {
    ...
    $(table).on('click', '.button-test-add', function() {
        ...
        constructForm('/api/stock/test/', {
            method: 'POST',
            fields: fields,
            title: 'Adicionar Resultado de Teste',
    ...
    $(table).on('click', '.button-test-edit', function() {
        ...
        constructForm(url, {
            ...
            title: 'Editar Resultado do Teste',
            onSuccess: reloadTestTable,
        });
    });
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Excluir resultado do teste</strong>
        </div>`;

        constructForm(url, {
            method: 'DELETE',
            title: 'Excluir Resultado do Teste',
            onSuccess: reloadTestTable,
            preFormContent: html,
        });
    });
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Usado no pedido de produção';
        ...
        text = `Instalado em Item de Estoque ${row.belongs_to}`;
        ...
        text = 'Atribuir para o Pedido de Venda';
        ...
        text = '<i>Sem local de estoque definido</i>';
        url = '';
    }
...
function loadStockTable(table, options) {
    ...
    if (!options.disableFilters) {
        ...
        setupFilterList(filterKey, table, filterTarget, {
            ...
            singular_name: 'item de estoque',
            plural_name: 'itens de estoque',
            barcode_actions: [
                {
                    icon: 'fa-sitemap',
                    label: 'scantolocation',
                    title: 'Escanear para local',
                    permission: 'stock.change',
            ...
            if (show_installed_items && row.installed_items > 0) {
                if (row.installed_items_received) {
                    ...
                    <a href='#' pk='${row.pk}' class='load-sub-items' id='load-sub-items-${row.pk}'>
                        <span class='fas fa-sync-alt' title='Carregar itens instalados'></span>
                    </a>`;
                }
            }
...
function loadStockLocationTable(table, options) {
    ...
    setupFilterList(filterKey, table, filterListElement, {
        ...
        singular_name: 'local de estoque',
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (!details || !Object.keys(details).length) {
                return `<small><em>Nenhuma mudança</em></small>`;
            }
            ...
            if (details.buildorder) {
                html += `<tr><th>Ordem de Produção</th>`;
                html += `<td>`;
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    <div class='alert alert-block alert-info'>
        ...
        <ul>
            <li>O Item de Estoque conecta a uma peça que é um BOM deste Item de Estoque</li>
    ...
    constructForm(
        `/api/stock/${stock_item_id}/install/`,
        {
            ...
            confirm: true,
            title: 'Instalar Item de Estoque',
            preFormContent: html,
            onSuccess: function(response) {
                if (options.onSuccess) {
