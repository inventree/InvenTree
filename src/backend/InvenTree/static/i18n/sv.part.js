    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Tillgänglig: ${part.unallocated_stock}`);
            } else {
                elements.push(`Tillgänglig: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Redigera mall');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Radera mall');
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Pack Quantity: ${pq} - Total Quantity: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Pack Quantity: ${pq} - Total Quantity: ${total}'></span>`;
                    }
...
function loadPartCategoryTable(table, options) {
    ...
    table.inventreeTable({
        ...
        method: 'get',
        formatNoMatches: function() {
            return 'Inga underkategorier hittades';
        },
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Ladda underkategorier'></span>
                                </a> `;
                        }
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.results && row.results > 0) {
                        html += `
                        <span class='badge bg-dark rounded-pill float-right' title='${row.results} resultat'>
                            ${row.results}
                        </span>`;
                    }
