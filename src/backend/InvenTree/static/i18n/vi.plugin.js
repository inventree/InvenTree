        ...
        columns: [
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.is_builtin) {
                        html += `<span class='badge bg-success rounded-pill badge-right'>Gắn liền</span>`;
                    }

                    if (row.is_sample) {
                        html += `<span class='badge bg-info rounded-pill badge-right'>Mẫu</span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.custom && !row.is_builtin && row.is_installed) {
                        if (row.active) {
                            buttons += makeIconButton('fa-stop-circle icon-red', 'btn-plugin-disable', row.key, 'Disable Plugin');
                        } else {
                            buttons += makeIconButton('fa-play-circle icon-green', 'btn-plugin-enable', row.key, 'Enable Plugin');
                        }
                    }
