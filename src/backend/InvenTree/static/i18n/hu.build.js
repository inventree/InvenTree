...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Készlet felszabadítása a gyártási kimenetből',
        );
    }
...
function deallocateStock(build_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-warning'>
    Biztosan szeretnéd a már lefoglalt készlet tételeket felszabadítani ebből a gyártási utasításból?
    </dvi>
    `;
    ...
    constructForm(url, {
        ...
        title: 'Készlet tételek felszabadítása',
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-success'>
    A kiválasztott gyártási kimenetek késznek lesznek jelölve
    </div>
...
function scrapBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    A kiválasztott gyártási kimenetek selejtnek lesznek jelölve
    <ul>
        <li>Selejtezett kimenetek elutasítottnak jelölve</li>
        <li>A lefoglalt készlet már nem lesz elérhető</li>
        <li>A befejezési státusza a gyártásnak nem fog változni</li>
    </ul>
    </div>
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    A kiválasztott gyártási kimenetek törölve lesznek
    <ul>
    <li>A gyártási kimenet adatai véglegesen törölve lesznek</li>
    <li>A lefoglalt készlet tételek újra készletre kerülnek</li>
    </ul>
    </div>
    <table class='table table-striped table-condensed' id='build-complete-table'>
        <thead>
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    setupFilterList('builditems', $(table), options.filterTarget || '#filter-list-incompletebuilditems', {
        ...
        custom_actions: [{
            label: 'buildoutput',
            icon: 'fa-tools',
            title: 'Gyártási kimenet műveletei',
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Köteg: ${row.batch})</small>`;
                    }
...
function loadBuildLineTable(table, build_id, options={}) {
    ...
    if (!options.data) {
        setupFilterList('buildlines', $(table), filterTarget, {
            ...
            singular_name: 'gyártás sor',
            plural_name: 'gyártás sorok',
        });
    }
    ...
    let table_options = {
        ...
        formatNoMatches: function() {
            return 'Nincsenek gyártási sorok';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.in_production && row.in_production > 0) {
                        icons += makeIconBadge('fa-tools icon-blue', `Gyártásban: ${formatDecimal(row.in_production)}`);
                    }

                    if (row.external_stock > 0) {
                        icons += makeIconBadge('fa-sitemap', `Külső raktárkészlet: ${row.external_stock}`);
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part_detail.trackable && !options.output) {
                        ...
                        return `
                        <div>
                            <em>Követett tétel</em>
                            <span title='Egyedileg nyilvántartott tételek lefoglalása egyedi gyártási kimenetekhez' class='fas fa-info-circle icon-blue' />
                        </div>`;
                    }
                    ...
                    if (row.allocated > 0) {
                        buttons += makeRemoveButton('button-unallocate', pk, 'Készlet foglalások törlése');
                    }
