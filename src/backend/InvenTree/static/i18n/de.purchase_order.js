...
function purchaseOrderFields(options={}) {
    ...
    if (options.duplicate_order) {
        ...
        fields.duplicate_extra_lines = {
            ...
            group: 'duplicate',
            type: 'boolean',
            label: 'Zusätzliche Positionen duplizieren',
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.is_complete) {
                    ...
                    <div class='alert alert-block alert-success'>
                        Alle Positionen wurden erhalten
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Diese Bestellung enthält Positionen, die nicht als empfangen markiert wurden.</br>
                        Fertigstellen dieser Bestellung bedeutet, dass sie ihre Positionen nicht länger bearbeiten können.
                    </div>`;
                }
...
function issuePurchaseOrder(order_id, options={}) {
    ...
    let html = `
    <div class='alert alert-block alert-warning'>
    Nachdem diese Bestellung platziert ist, können die Positionen nicht länger bearbeitet werden.
    </div>`;
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                ...
                <th style='min-width: 50px;'>Zusammenfügen</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                ...
                Erhaltene Menge: <span class='pack_received_quantity' id='items_received_quantity_${pk}'>${received}</span> ${units}
            </div>`;
        }
        ...
        if (global_settings.BARCODE_ENABLE) {
            buttons += makeIconButton('fa-qrcode', 'button-row-add-barcode', pk, 'Barcode hinzufügen');
            buttons += makeIconButton('fa-unlink icon-red', 'button-row-remove-barcode', pk, 'Barcode entfernen', {hidden: true});
        }

        buttons += makeIconButton('fa-sitemap', 'button-row-add-location', pk, 'Lagerort angeben', {
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Empfang der Artikel bestätigen',
        title: 'Bestellartikel erhalten',
        afterRender: function(fields, opts) {
            ...
            if (global_settings.BARCODE_ENABLE) {
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Artikel-Barcode scannen', {
                        details: 'Scanne den Barcode am erhaltenen Artikel (darf nicht mit einem existierenden Lagerartikel übereinstimmen)',
                        ...
                        onError400: function(response, barcode_options) {
                            if (response.barcode_data && response.barcode_hash) {
                                ...
                                showBarcodeMessage(barcode_options.modal, 'Ungültige Barcode-Daten');
                            }
                        }
                    });
                });
...
function loadPurchaseOrderLineItemTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'Keine Postionen gefunden';
        },
        queryParams: filters,
        original: options.params,
