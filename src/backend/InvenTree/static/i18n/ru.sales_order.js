    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                Нет зарезервированных складских позиций для этого отправления
                </div>
                `;
            } else {
                html = `
                Следующие складские позиции будут отправлены
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            ...
            html += `
            Нет складских позиций зарезервированных для ожидающих отправлений
            `;
        }
    ...
    $(table).inventreeTable({
        ...
        columns: [
            ...
            },
            {
                sortable: true,
                field: 'reference',
                title: 'Заказ на продажу',
                formatter: function(value, row) {
                    var html = renderLink(value, `/order/sales-order/${row.pk}/`);

                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Заказ просрочен');
                    }
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Редактировать отправление');
        ...
        html += makeDeleteButton('button-shipment-delete', pk, 'Удалить отправление', {disabled: !enable_delete});
    ...
    function setupShipmentCallbacks() {
        ...
        $(table).find('.button-shipment-edit').click(function() {
            ...
            constructForm(`/api/order/so/shipment/${pk}/`, {
                fields: fields,
                title: 'Редактировать отправление',
                refreshTable: table,
            });
        });
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        title: 'Зарезервировать складские позиции для заказа на продажу',
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    var text = `Складская позиция ${row.item}`;
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    ...
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Зарезервировать Остатки');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Закупить запасы');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Запасы производства');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Рассчитать стоимость');

            if (options.allow_edit) {
                buttons += makeCopyButton('button-duplicate', pk, 'Duplicate line item');
                buttons += makeEditButton('button-edit', pk, 'Редактировать Позицию');
            }

            if (options.allow_delete) {
                var delete_disabled = false;

                var title = 'Удалить позицию';
