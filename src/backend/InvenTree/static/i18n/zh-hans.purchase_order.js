...
function orderParts(parts_list, options={}) {
    ...
    function renderPart(part, opts={}) {
        ...
        var supplier_part_prefix = `
            <button type='button' class='input-group-text button-row-new-sp' pk='${pk}' title='新建供应商零件'>
        ...
        var purchase_order_prefix = `
            <button type='button' class='input-group-text button-row-new-po' pk='${pk}' title='新建采购订单'>
                <span class='fas fa-plus-circle icon-green'></span>
            </button>
        `;
    ...
    <table class='table table-striped table-condensed' id='order-parts-table'>
        <thead>
            <tr>
                <th>零件</th>
                <th style='min-width: 300px;'>供应商零件</th>
                <th style='min-width: 300px;'>采购订单</th>
                <th style='min-width: 50px;'>數量</th>
                <th style='min-width: 50px;'>合并</th>
                <th><!-- Actions --></th>
            </tr>
        </thead>
        <tbody>
    ...
    function renderLineItem(line_item, opts={}) {
        ...
        if (native_pack_quantity != 1) {
            ...
            <div class='alert alert-small alert-block alert-info'>
                ...
                接收数量: <span class='pack_received_quantity' id='items_received_quantity_${pk}'>${received}</span> ${units}
            </div>`;
        }
        ...
        if (global_settings.BARCODE_ENABLE) {
            buttons += makeIconButton('fa-qrcode', 'button-row-add-barcode', pk, '添加条形码');
            buttons += makeIconButton('fa-unlink icon-red', 'button-row-remove-barcode', pk, '移除条形码', {hidden: true});
        }

        buttons += makeIconButton('fa-sitemap', 'button-row-add-location', pk, '指定位置', {
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th>订单编码</th>
                <th>已接收</th>
                <th style='min-width: 50px;'>接收数量</th>
        ...
        afterRender: function(fields, opts) {
            ...
            if (global_settings.BARCODE_ENABLE) {
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('扫描商品条形码', {
                        details: '扫描进货条形码 (必须与任何现有的库存条目不匹配)',
                        onScan: function(response, barcode_options) {
                            ...
                            showBarcodeMessage(barcode_options.modal, '条形码匹配现有项目');
                        },
                        onError400: function(response, barcode_options) {
                            if (response.barcode_data && response.barcode_hash) {
                                ...
                                showBarcodeMessage(barcode_options.modal, '条形码数据无效');
                            }
                        }
                    });
                });
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.target_date) {
                        ...
                        if (row.overdue) {
                            html += makeIconBadge('fa-calendar-times icon-red', '此行项目已逾期');
                        }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, '收到行项目');
                    }

                    if (options.allow_edit) {
                        buttons += makeCopyButton('button-line-duplicate', pk, '复制行项目');
                        buttons += makeEditButton('button-line-edit', pk, '编辑行项目');
                        buttons += makeDeleteButton('button-line-delete', pk, '删除行项目');
                    }
