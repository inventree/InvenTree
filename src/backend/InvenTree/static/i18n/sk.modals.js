...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Zrušiť</button>`;
    }

    // Add in a "submit" button
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>Odoslať</button>`;
    }
    ...
    modalSetSubmitText(modal_name, options.submitText || 'Odoslať');
    modalSetCloseText(modal_name, options.closeText || 'Zrušiť');
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.closeText = options.cancel_text || 'Zrušiť';
...
function openModal(options) {
    ...
    var submit_text = options.submit_text || 'Odoslať';
    var close_text = options.close_text || 'Zatvoriť';
