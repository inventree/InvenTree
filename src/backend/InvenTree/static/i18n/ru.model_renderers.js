...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        stock_detail = `<span class='badge rounded-pill bg-danger'>Нет запасов</span>`;
    } else {
        if (data.serial && data.quantity == 1) {
            ...
            if (render_available_quantity) {
                ...
                stock_detail = `Доступно: ${available}`;
            } else {
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>Неактивный</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Отправление ${data.reference}`,
        },
        parameters
    );
}
