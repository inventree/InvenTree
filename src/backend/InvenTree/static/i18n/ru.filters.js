...
function generateAvailableFilterList(tableKey) {
    ...
    html += `<option value=''>Выбрать фильтр</option>`;
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        let print_buttons = `
        <div class='btn-group' role='group'>
        <button id='printing-options' title='Действия печати' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
            <span class='fas fa-print'></span> <span class='caret'></span>
        </button>
        <ul class='dropdown-menu' role='menu'>`;

        if (labels_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-labels-${tableKey}'><span class='fas fa-tag'></span> Печать этикеток</a></li>`;
        }

        if (report_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-report-${tableKey}'><span class='fas fa-file-pdf'></span> Распечатать отчеты</a></li>`;
        }
    ...
    element.find(`#${add}`).click(function clicked() {
        ...
        if (!addClicked) {
            ...
            html += `<button title='Создать фильтр' class='btn btn-outline-secondary filter-button' id='${make}'><span class='fas fa-plus'></span></button>`;
            html += `</div>`;
