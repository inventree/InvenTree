...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        stock_detail = `<span class='badge rounded-pill bg-danger'>无库存</span>`;
    } else {
        if (data.serial && data.quantity == 1) {
            ...
            if (render_available_quantity) {
                ...
                stock_detail = `可用數量: ${available}`;
            } else {
                stock_detail = `數量: ${data.quantity}`;
            }
        }

        if (data.batch) {
            stock_detail += ` - <small>队列: ${data.batch}</small>`;
        }
    }
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>未激活</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `配送 ${data.reference}`,
        },
        parameters
    );
}
