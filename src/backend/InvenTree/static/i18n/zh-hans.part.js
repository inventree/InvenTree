...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        ...
        follow: true,
        persist: true,
        persistMessage: '在此类别之后创建新类别',
        successMessage: '零件类别已创建'
    });
}
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        title: '创建零件',
        persist: true,
        persistMessage: '在此零件之后创建另一个零件',
        successMessage: '零件创建成功',
...
function deletePart(pk, options={}) {
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            ...
            删除此零件无法撤销
            <ul>
            <li>此零件的任何库存项目都将被删除</li>
            <li>此零件将从任何物料清单中删除</li>
            <li>此零件的所有制造商和供应商信息都将被删除</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', '您已订阅此项目的通知');

                            showMessage('您已订阅此项目的通知', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', '订阅此项目的通知');

                            showMessage('您已取消订阅此项目的通知', {
                                style: 'warning',
                            });
                        }
                    }
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`低库存: ${part.total_in_stock}`);
            ...
            if (options.hideTotalStock) {
                elements.push(`可用數量: ${part.unallocated_stock}`);
            } else {
                elements.push(`可用數量: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
    ...
    if (part.ordering) {
        elements.push(`已订购: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`正在生产: ${part.building}`);
    }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>单位: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.virtual) {
        html += makeIconBadge('fa-ghost', '虚拟零件');
    }
    ...
    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', '订阅的零件');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', '可销售的零件');
    }
    ...
    if (part.locked) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>已锁定</span>`;
    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('button-edit-stocktake', row.pk, '编辑盘点条目');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('button-delete-stocktake', row.pk, '删除盘点条目');
                    }
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='包括变体库存' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='包装数量: ${pq} - 总数量: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>已接收</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, '收到行项目');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, '删除零件关系');
...
function partGridTile(part) {
    ...
    rows += `<tr><td><b>庫存</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>已订购</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>正在生产</b></td><td>${part.building} ${units}</td></tr>`;
    }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='加载子类别'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', '已订阅类别');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.results && row.results > 0) {
                        html += `
                        <span class='badge bg-dark rounded-pill float-right' title='${row.results} 结果'>
                            ${row.results}
                        </span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        html += makeEditButton('button-test-edit', pk, '编辑测试模板');
                        html += makeDeleteButton('button-test-delete', pk, '删除测试模板');

                        return wrapButtons(html);
                    } else {
                        var text = '此测试是为父零件定义的';
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        ...
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', '指定日期已过');
                    }

                    var quantity_string = entry.quantity + entry.speculative_quantity;

                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', '可指定的');
                    }
