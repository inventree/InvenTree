...
function partGroups() {
    ...
    return {
        attributes: {
            title: 'Atributos da Peça',
            collapsible: true,
        },
        create: {
            title: 'Opções de Criação de Peça',
            collapsible: true,
        },
        duplicate: {
            title: 'Opções de Duplicação de Peça',
            collapsible: true,
        },
        initial_stock: {
...
function partFields(options={}) {
    ...
    var fields = {
        category: {
            secondary: {
                title: 'Adicionar Categoria de Peça',
                fields: function() {
...
function categoryFields(options={}) {
    let fields = {
        parent: {
            label: 'Parent',
            help_text: 'Categoria de peça pai',
            required: false,
            tree_picker: {
                url: '/api/part/category/tree/',
            },
        ...
        structural: {},
        icon: {
            help_text: `Ícone (opcional) - Explorar todos os ícones disponíveis em <a href="https://tabler.io/icons" target="_blank" rel="noopener noreferrer">Tabler Icons</a>.`,
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        ...
        follow: true,
        persist: true,
        persistMessage: 'Criar nova categoria após esta',
        successMessage: 'Categoria da peça criada'
    });
}
...
function editCategory(pk) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Editar Categoria da Peça',
        reload: true,
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    Você tem certeza que deseja excluir essa categoria de peça?
    </div>`;
    var subChoices = [
        {
            value: 0,
            display_name: 'Mover para categoria parental',
        },
        {
            value: 1,
    ...
    constructForm(url, {
        title: 'Excluir Categoria de Peça',
        method: 'DELETE',
        fields: {
            'delete_parts': {
                label: 'Ação para peças nesta categoria',
                choices: subChoices,
                type: 'choice'
            },
            'delete_child_categories': {
                label: 'Ação para categorias filhas',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        title: 'Criar Peça',
        persist: true,
        persistMessage: 'Criar outra peça após esta',
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            if (part.active) {
                showAlertDialog(
                    'Peça Ativa',
                    'Peça não pode ser excluída enquanto estiver ativa',
                    {
                        alert_style: 'danger',
                    }
                );
            ...
            Excluir esta peça não é reversível
            <ul>
            <li>Qualquer item de estoque desta peça será excluído</li>
            <li>Esta peça será removida de quaisquer Lista de Materiais (BOM)</li>
            <li>Toda informação de fabricante e fornecedor dessa peça será excluída</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Você está inscrito para receber notificações para este item');

                            showMessage('Você se inscreveu para notificações deste item', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Inscreva-se para receber notificações deste item');

                            showMessage('Você descadastrou para notificações deste item', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Validando a BOM irá marcar como cada linha válida
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Validar Lista de Materiais (BOM)',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('Lista de Materiais Validada');
        }
    });
}
...
function duplicateBom(part_id, options={}) {
    ...
    constructForm(`/api/part/${part_id}/bom-copy/`, {
        ...
        confirm: true,
        title: 'Copiar Lista de Materiais (BOM)',
        onSuccess: function(response) {
            if (options.success) {
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            elements.push(`Estoque baixo: ${part.total_in_stock}`);
        } else if (part.unallocated_stock <= 0) {
            // There is no available stock at all
            elements.push(`Nenhum estoque disponível`);
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Unidade: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Peça virtual');
    }

    if (part.is_template) {
        html += makeIconBadge('fa-clone', 'Modelo de peça');
    }

    if (part.assembly) {
        html += makeIconBadge('fa-tools', 'Peça montada');
    }

    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Peça inscrita');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Peça vendível');
    }
    ...
    if (part.locked) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Locked</span>`;
    }
...
function loadPartVariantTable(table, partId, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Incluir estoque de variantes' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
    ...
    table.inventreeTable({
        ...
        queryParams: filters,
        formatNoMatches: function() {
            return 'Nenhuma variante encontrada';
        },
        columns: cols,
        treeEnable: true,
        rootParentId: partId,
...
function editPartParameter(param_id, options={}) {
    options.fields = partParameterFields();
    options.title = 'Editar Parâmetro';
    options.focus = 'data';
...
function loadPartParameterTable(table, options) {
    ...
    $(table).inventreeTable({
        ...
        groupBy: false,
        formatNoMatches: function() {
            return 'Nenhum parâmetro encontrado';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    html += makeEditButton('button-parameter-edit', pk, 'Editar parâmetro');
                    html += makeDeleteButton('button-parameter-delete', pk, 'Excluir parâmetro');
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhum modelo parâmetro de peça encontrado';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Editar Modelo');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Excluir Modelo');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Editar Modelo de Parâmetro da Peça',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
            Quaisquer parâmetros que se referencie este modelo serão excluídos
        </div>`;
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Excluir Modelo de Parâmetro de Peça',
                refreshTable: table,
            }
        );
    });
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Quantidade de embalagens: ${pq} - Quantidade Total: ${total}`
                        );
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (overdue) {
                        html += makeIconBadge(
                            ...
                            'Este item de linha está atrasado',
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Quantidade de embalagens: ${pq} - Quantidade Total: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        ...
                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Receber item de linha');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Excluir relação de peças');
...
function setPartCategory(data, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        Definir a categoria das peças selecionadas
    </div>
    `;

    constructForm('/api/part/change_category/',{
        title: 'Definir Categoria da Peça',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {
    ...
    columns.push({
        ...
        formatter: function(value, row) {
            if (row.category && row.category_detail) {
                ...
                return '<em>Nenhuma categoria</em>';
            }
        }
    });
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Carregar Subcategorias'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Categoria inscrita');
                    }
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Nenhum modelo de teste corresponde à consulta';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.results && row.results > 0) {
                        html += `
                        <span class='badge bg-dark rounded-pill float-right' title='${row.results} results'>
                            ${row.results}
                        </span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        html += makeEditButton('button-test-edit', pk, 'Edit test template');
                        html += makeDeleteButton('button-test-delete', pk, 'Delete test template');

                        return wrapButtons(html);
                    } else {
                        var text = 'Este teste é definido para uma peça parental';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Editar Modelo de Resultado de Teste',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Excluir Modelo de Resultado de Teste',
                    refreshTable: table,
                });
            });
        }
...
function loadPartSchedulingChart(canvas_id, part_id) {
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>Nenhuma data especificada</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', 'Nenhuma data especificada');
                    } else if (date < today) {
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', 'Data especificada está no passado');
                    }
    ...
    if (n_entries < 1) {
        ...
        var message = `
        <div class='alert alert-block alert-info'>
            Nenhuma informação de agendamento para esta peça.
        </div>`;

        if (was_error) {
            message = `
                <div class='alert alert-block alert-danger'>
                    Erro ao obter informações de agendamento para esta peça.
                </div>
            `;
        }
    ...
    var data = {
        datasets: [
            {
                label: 'Quantidades de Estoque Agendadas',
