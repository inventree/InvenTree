...
function createNewModal(options={}) {
    ...
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>Submeter</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Título do Formulário');
    modalSetSubmitText(modal_name, options.submitText || 'Submeter');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Aguardando o servidor...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Mostrar Informação do Erro</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Aceitar';
...
function openModal(options) {
    ...
    if (options.title) {
        ...
        modalSetTitle(modal, 'Carregando dados...');
    }
    ...
    var submit_text = options.submit_text || 'Submeter';
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('Resposta inválida do servidor', 'Dado de formulário faltando na resposta do servidor');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Erro ao postar os dados de formulários', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Resposta do servidor inválida', 'Dados de formulário faltando na resposta JSON');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Sem Resposta',
                    'Sem resposta do servidor InvenTree',
                );
            } else if (xhr.status == 400) {
                showAlertDialog(
                    'Erro 400: Requisição Ruim',
                    'Servidor retornou o código de erro 400',
                );
            } else if (xhr.status == 401) {
                showAlertDialog(
                    'Erro 401: Não Autenticado',
                    'Credenciais de autenticação não fornecidas',
                );
            } else if (xhr.status == 403) {
                showAlertDialog(
                    'Erro 403: Permissão Negada',
                    'Você não tem as permissões necessárias para acessar esta função',
                );
            } else if (xhr.status == 404) {
                showAlertDialog(
                    'Erro 404: Recurso Não Encontrado',
                    'O recurso requisitado não pôde ser encontrado no servidor',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Erro 408: Tempo Limite',
                    'Tempo limite da conexão atingido ao solicitar dados do servidor',
                );
            } else {
                showAlertDialog('Erro ao pedir dados de formulário', renderErrorMessage(xhr));
            }
