        ...
        columns: [
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.is_builtin) {
                        html += `<span class='badge bg-success rounded-pill badge-right'>Встроенный</span>`;
                    }

                    if (row.is_sample) {
                        html += `<span class='badge bg-info rounded-pill badge-right'>Образец</span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.custom && !row.is_builtin && row.is_installed) {
                        if (row.active) {
                            buttons += makeIconButton('fa-stop-circle icon-red', 'btn-plugin-disable', row.key, 'Отключить плагин');
                        } else {
                            buttons += makeIconButton('fa-play-circle icon-green', 'btn-plugin-enable', row.key, 'Включить плагин');
                        }
                    }
...
function activatePlugin(plugin_id, active=true) {
    ...
    constructForm(null, {
        title: active ? 'Включить плагин' : 'Отключить плагин',
        preFormContent: html,
        confirm: true,
        submitText: active ? 'Включить' : 'Отключено',
            ...
            inventreePut(
                ...
                },
                {
                    ...
                    success: function() {
                        ...
                        addCachedAlert('Плагин обновлен', {style: 'success'});
                        location.reload();
                    },
                    error: function(xhr) {
                        $(opts.modal).modal('hide');
