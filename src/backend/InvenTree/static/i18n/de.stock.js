...
function stockLocationTypeFields() {
    const fields = {
        ...
        description: {},
        icon: {
            help_text: `Icon (optional) - alle verfügbaren Icons einsehbar auf <a href="https://tabler.io/icons" target="_blank" rel="noopener noreferrer">Tabler Icons</a>.`,
...
function editStockLocation(pk, options={}) {
    ...
    options.title = 'Lagerort bearbeiten';
...
function createStockLocation(options={}) {
    ...
    options.method = 'POST';
    options.fields = stockLocationFields(options);
    options.title = 'Lagerort hinzufügen';
    options.persist = true;
    options.persistMessage = 'Weiteren Lagerort nach diesem erstellen';
    options.successMessage = 'Lagerort erstellt';
...
function deleteStockLocation(pk, options={}) {
    ...
    Soll dieser Lagerort gelöscht werden?
    </div>
    `;
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'Zum übergeordneten Lagerort verschieben',
        },
        {
            value: 1,
...
function stockItemFields(options={}) {
    var fields = {
        ...
        use_pack_size: {
            help_text: 'Angegebene Menge als Packungen anstatt einzelner Artikel hinzufügen',
        },
        location: {
            icon: 'fa-sitemap',
            filters: {
...
function duplicateStockItem(pk, options) {
    ...
    inventreeGet(`/api/stock/${pk}/`, {}, {
        success: function(data) {
            ...
            options.method = 'POST';
            options.title = 'Lagerartikel duplizieren';
...
function deleteStockItem(pk, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Soll dieser Lagerartikel gelöscht werden?
    </div>`;
...
function createNewStockItem(options={}) {
    ...
    options.persist = true;
    options.persistMessage = 'Weiteres Teil nach diesem erstellen';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                addCachedAlert('Neuen Lagerartikel erstellen', {
                    icon: 'fas fa-boxes',
                });
                ...
                showMessage('Mehrere Lagerartikel wurden erstellt', {
                    icon: 'fas fa-boxes',
                    details: details,
                });
                    ...
                    success: function(response) {
                        if (response.length == 0) {
                            ...
                            handleFormErrors(
                                {
                                    'serial': [
                                        'Mehr als ein übereinstimmendes Ergebnis gefunden',
                                    ]
                                }, fields, opts
                            );
                        } else {
...
function assignStockToCustomer(items, options={}) {
    ...
    constructForm('/api/stock/assign/', {
        ...
        title: 'Lagerbestand einem Kunden zuweisen',
...
function mergeStockItems(items, options={}) {
    ...
    constructForm('/api/stock/merge/', {
        ...
        confirm: true,
        confirmMessage: 'Bestandszusammenführung bestätigen',
        title: 'Lagerartikel zusammenführen',
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
                var pk = $(this).attr('pk');
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantityString += ` - <small>Losnummer: ${item.batch}</small>`;
        }
    ...
    if (itemCount == 0) {
        showAlertDialog(
            ...
            'Wähle mindestens einen verfügbaren Lagerartikel aus',
        );
    ...
    constructForm(url, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Bestandsanpassung bestätigen',
        title: formTitle,
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.templateId, 'Testergebnis hinzufügen');

        if (!grouped && row.result != null) {
            html += makeEditButton('button-test-edit', row.testId, 'Testergebnis bearbeiten');
            html += makeDeleteButton('button-test-delete', row.testId, 'Testergebnis löschen');
        }
    ...
    $(table).on('click', '.button-test-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
        <strong>Testergebnis löschen</strong>
        </div>`;
...
function loadStockTable(table, options) {
    ...
    if (!options.disableFilters) {
        ...
        setupFilterList(filterKey, table, filterTarget, {
            ...
            singular_name: 'Lagerartikel',
            plural_name: 'Lagerartikel',
            barcode_actions: [
                {
                    icon: 'fa-sitemap',
                    label: 'scantolocation',
                    title: 'Zu Lagerort einscannen',
                    permission: 'stock.change',
            ...
            if (show_installed_items && row.installed_items > 0) {
                if (row.installed_items_received) {
                    ...
                    <a href='#' pk='${row.pk}' class='load-sub-items' id='load-sub-items-${row.pk}'>
                        <span class='fas fa-sync-alt' title='Installierte Artikel laden'></span>
                    </a>`;
                }
            }
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.sales_order) {
                ...
                html += makeIconBadge('fa-box', 'Lagerartikel in anderem Artikel verbaut');
            } else if (row.consumed_by) {
                html += makeIconBadge('fa-tools', 'Lagerbestand wurde durch einen Bauauftrag verbraucht');
            }
            ...
            if (row.status == stockCodes.REJECTED.key) {
                html += makeIconBadge('fa-times-circle icon-red', 'Lagerartikel wurde zurückgewiesen');
            } else if (row.status == stockCodes.LOST.key) {
                html += makeIconBadge('fa-question-circle', 'Lagerartikel ist verloren');
            } else if (row.status == stockCodes.DESTROYED.key) {
                html += makeIconBadge('fa-skull-crossbones', 'Lagerartikel ist zerstört');
            }

            if (row.quantity <= 0) {
                html += `<span class='badge badge-right rounded-pill bg-danger'>Aufgebraucht</span>`;
            }
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Keine zur Anfrage passenden Lagerartikel gefunden';
        },
...
function loadStockLocationTable(table, options) {
    ...
    setupFilterList(filterKey, table, filterListElement, {
        ...
        singular_name: 'Lagerort',
        plural_name: 'Lagerorte',
    });
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-location'>
                                    <span class='fas fa-sync-alt' title='Untergeordnete Lagerorte laden'></span>
                                </a> `;
                        }
                    }
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (!details || !Object.keys(details).length) {
                return `<small><em>Keine Änderungen</em></small>`;
            }
            ...
            if (details.part) {
                ...
                if (details.part_detail) {
                    ...
                    html += `Teileinformationen nicht verfügbar`;
                }
            ...
            if (details.location) {
                ...
                if (details.location_detail) {
                    ...
                    html += `<i>Lagerort existiert nicht mehr</i>`;
                }
            ...
            if (details.buildorder) {
                ...
                if (details.buildorder_detail) {
                    ...
                    html += `<i>Bauauftrag existiert nicht mehr</i>`;
                }
            }
            ...
            if (details.salesorder) {
                ...
                if (details.salesorder_detail) {
                    ...
                    html += `<em>Auftrag existiert nicht mehr</em>`;
                }
            ...
            if (details.returnorder) {
                html += `<tr><th>Rücksendeauftrag</th>`;
                html += '<td>';
                ...
                if (details.returnorder_detail) {
                    ...
                    html += `<em>Rücksendebestellung existiert nicht mehr</em>`;
                }
...
function loadInstalledInTable(table, options) {
    ...
    */
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine installierten Artikel';
        },
        columns: [
            {
                field: 'part',
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.can_edit) {
                        html += makeIconButton('fa-unlink', 'button-uninstall', pk, 'Lagerartikel deinstallieren');
                    }
...
function uninstallStockItem(installed_item_id, options={}) {
    ...
    constructForm(
        `/api/stock/${installed_item_id}/uninstall/`,
        {
            confirm: true,
            method: 'POST',
            title: 'Lagerartikel deinstallieren',
            fields: {
                location: {
                    icon: 'fa-sitemap',
                    filters: {
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        <strong>Einen anderen Lagerartikel in dieses Teil installieren</strong><br>
        Lagerartikel können nur installiert werden wenn folgende Kriterien erfüllt werden:<br>
        <ul>
            <li>Der Lagerartikel ist mit einem Teil verknüpft das in der Stückliste für diesen Lagerartikel ist</li>
...
function setStockStatus(items, options={}) {
    ...
    if (items.length == 0) {
        showAlertDialog(
            'Lagerartikel auswählen',
            'Wählen Sie einen oder mehrere Bestandteile aus'
        );
        return;
    }
    ...
    Lagerartikel auswählen: ${items.length}
    </div>`;
