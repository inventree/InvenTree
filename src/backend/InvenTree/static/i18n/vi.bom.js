...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeRemoveButton('button-row-remove', idx, 'Xóa hàng');
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    var html = `
    <div class='alert alert-block'>
    <strong>Sản phẩm cơ bản</strong><hr>
...
function loadBomTable(table, options={}) {
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Vật tư tiêu hao)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Tuỳ chọn)</small>';
            }
    ...
    if (options.editable) {
        ...
        cols.push({
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Thêm mục BOM' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Thêm mục BOM
                </button>
                `;
            }
        });
