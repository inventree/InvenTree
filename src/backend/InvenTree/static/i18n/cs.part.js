...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Dostupné: ${part.unallocated_stock}`);
            } else {
                elements.push(`Dostupné: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
    ...
    if (part.ordering) {
        elements.push(`Na objednávku: ${part.ordering}`);
    }
...
function makePartIcons(part) {
    ...
    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Neaktivní</span> `;
    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Počet kusů v balení: ${pq} - Total Quantity: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Počet kusů v balení: ${pq} - Total Quantity: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Doručeno</span>`;
...
function partGridTile(part) {
    ...
    if (part.ordering) {
        rows += `<tr><td><b>Na objednávku</b></td><td>${part.ordering} ${units}</td></tr>`;
    }
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Množství: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
