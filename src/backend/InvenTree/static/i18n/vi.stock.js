...
function createNewStockItem(options={}) {
    ...
    options.title = 'Hàng trong kho mới';
    options.method = 'POST';
    ...
    options.successMessage = 'Kho hàng đã được khởi tạo';
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            if (response.pk) {
                ...
                <br>Số sê-ri: ${response.serial_numbers}
                `;
...
function adjustStock(action, items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantityString += ` - <small>Hàng loạt: ${item.batch}</small>`;
        }
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Tiêu hao bởi đơn đặt bản dựng';
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Kho hàng đã được gộp', {
                            style: 'success',
                        });
                    }
                });
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (details.returnorder) {
                html += `<tr><th>Đơn hàng trả lại</th>`;
                html += '<td>';
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    constructForm(
        `/api/stock/${stock_item_id}/install/`,
        {
            ...
            confirm: true,
            title: 'Lắp đặt hàng hóa trong kho',
            preFormContent: html,
            onSuccess: function(response) {
                if (options.onSuccess) {
