...
function insertConfirmButton(options) {
    ...
    var message = options.confirmMessage || 'Potvrdiť';
...
function initializeRelatedField(field, fields, options={}) {
    ...
    if(field.tree_picker) {
        ...
        button.on("click", () => {
            ...
            const title = 'Select' + " " + options.actions[name].label;
            const content = `
                <div class="mb-1">
                    <div class="input-group mb-2">
                        <input class="form-control" type="text" id="${name}_tree_search" placeholder="Vyhľadávanie ${options.actions[name].label}..." />
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <button class='input-group-text form-clear' id='clear_${field_name}' title='Clear input'>
