...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        stock_detail = `<span class='badge rounded-pill bg-danger'>Hết hàng</span>`;
    } else {
        if (data.serial && data.quantity == 1) {
            stock_detail = `Số sê-ri: ${data.serial}`;
        } else {
            if (render_available_quantity) {
                var available = data.quantity - data.allocated;
                stock_detail = `Có sẵn: ${available}`;
            } else {
        ...
        if (data.batch) {
            stock_detail += ` - <small>Hàng loạt: ${data.batch}</small>`;
        }
    }
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>Không hoạt động</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Vận chuyển ${data.reference}`,
        },
        parameters
    );
}
