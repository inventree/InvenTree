        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'O pedido está atrasado');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        html += makeEditButton('button-allocation-edit', pk, 'Editar alocação de estoque');
                        html += makeDeleteButton('button-allocation-delete', pk, 'Excluir alocação de estoque');
                    }
...
function loadSalesOrderLineItemTable(table, options={}) {
        ...
        },
        {
            ...
            formatter: function(value, row) {
                if (row.target_date) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times', 'Este item de linha está atrasado');
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (available > 0) {
                        ...
                        if (row.available_variant_stock && row.available_variant_stock > 0) {
                            html += makeIconBadge('fa-info-circle icon-blue', 'Incluir estoque de variantes');
                        }
                    } else {
                        html += `<span class='badge rounded-pill bg-danger'>Nenhum Estoque Disponível</span>`;
                    }

                    if (required > 0) {
                        if (available >= required) {
                            html += makeIconBadge('fa-check-circle icon-green', 'Estoque suficiente disponível');
                        } else {
                            html += makeIconBadge('fa-times-circle icon-red', 'Estoque insuficiente disponível');
                        }
                    }
        ...
        formatter: function(value, row, index, field) {
            ...
            if (row.part && row.part_detail) {
                ...
                if (options.allow_edit && (row.shipped < row.quantity)) {
                    if (part.trackable) {
                        buttons += makeIconButton('fa-hashtag icon-green', 'button-add-by-sn', pk, 'Alocar números de série');
                    }
                    buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-add', pk, 'Alocar Estoque');
                    if (part.purchaseable) {
                        buttons += makeIconButton('fa-shopping-cart', 'button-buy', row.part, 'Comprar estoque');
                    }

                    if (part.assembly) {
                        buttons += makeIconButton('fa-tools', 'button-build', row.part, 'Estoque de produção');
                    }
                }
            }

            buttons += makeIconButton('fa-dollar-sign icon-green', 'button-price', pk, 'Calcular preço');
            ...
            launchModalForm(
                '/order/purchase-order/pricing/',
                {
                    ...
                    buttons: [
                        {
                            name: 'update_price',
                            title: 'Atualizar Preço Unitário'
                        },
                    ],
                    success: reloadTable,
                }
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhum item de linha correspondente';
        },
        queryParams: filters,
        original: options.params,
        url: options.url,
