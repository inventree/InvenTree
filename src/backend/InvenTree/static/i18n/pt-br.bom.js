...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Mostrar dados da linha');
...
function exportBom(part_id, options={}) {
    ...
    constructFormBody({}, {
        ...
        fields: {
            ...
            cascade: {
                label: 'BOM Multinível',
                help_text: 'Incluir dados BOM para sub-montagens',
                type: 'boolean',
                value: inventreeLoad('bom-export-cascading', true),
            },
            levels: {
                label: 'Níveis',
                help_text: 'Selecione o número máximo de níveis BOM para exportar (0= todos os níveis)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Incluir Peças Alternativas',
                help_text: 'Incluir peças alternativas na BOM exportada',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Incluir Parâmetros de Dados',
                help_text: 'Incluir dados do parâmetro da peça na BOM exportada',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Incluir Dados do Estoque',
                help_text: 'Incluir dados do estoque da peça na BOM exportada',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Incluir Dados do Fabricante',
                help_text: 'Incluir dados da peça do fabricante na BOM exportada',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Incluir Dodos do Fornecedor',
                help_text: 'Incluir dados da peça do fornecedor na BOM exportada',
                type: 'boolean',
                value: inventreeLoad('bom-export-supplier_data', false),
            },
            pricing_data: {
                label: 'Incluir Dados de Preço',
                help_text: 'Incluir dados de preço na BOM exportada',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Remover peça substituta');
    ...
    <div class='alert alert-success alert-block'>
        Selecione e adicione uma nova peça substituída usando a entrada abaixo
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            Tem certeza que deseja remover este link de peça substituta?
            </div>
            `;

            constructForm(`/api/bom/substitute/${pk}/`, {
                method: 'DELETE',
                title: 'Remover Peça Substituta',
                preFormContent: pre,
                confirm: true,
                onSuccess: function() {
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'Fechar',
        submitText: 'Adicionar Substituto',
        title: 'Editar Itens Substitutos da BOM',
...
function deleteBomItems(items, options={}) {
    ...
    Todos os itens selecionados da BOM serão apagados
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Apagar itens selecionados da BOM?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                    ...
                    if (row.sub_assembly_received) {
                        ...
                        html += `
                            <a href='#' pk='${row.pk}' class='load-sub-assembly' id='load-sub-assembly-${row.pk}'>
                                <span class='fas fa-sync-alt' title='Carregar BOM para a submontagem'></span>
                            </a> `;
                    }
                }
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Substitutos Disponíveis');
                }
            ...
            if (any_pricing) {
                ...
                if (complete_pricing) {
                    html += makeIconBadge(
                        ...
                        'Preços da BOM estão completos',
                    );
                } else {
                    html += makeIconBadge(
                        'fa-exclamation-circle icon-yellow',
                        'Preços da BOM estão incompletos',
                    );
                }
                ...
                let html = '<em>Nenhum preço disponível</em>';
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (external_stock > 0) {
                text += makeIconBadge('fa-sitemap', `External stock: ${external_stock}`);
            }

            if (available_stock <= 0) {
                text += makeIconBadge('fa-times-circle icon-red', 'Nenhum Estoque Disponível');
            } else {
                var extra = '';
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.consumable) {
                    return `<em>Itens consumíveis</em>`;
                }
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Validar Item da BOM');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'Esta linha foi validada', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Editar peças substitutas');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Editar Item da BOM');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Apagar Item da BOM');
    ...
    table.inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhum item da BOM encontrado';
        },
        queryParams: filters,
        original: params,
        columns: cols,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Herdado da BOM paternal)</em>`;
                    }
