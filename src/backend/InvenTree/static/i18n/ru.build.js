...
function editBuildOrder(pk) {
    ...
    constructForm(`/api/build/${pk}/`, {
        fields: fields,
        reload: true,
        title: 'Редактировать заказ на производство',
    });
}
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.has_allocated_stock) {
                    ...
                    <div class='alert alert-block alert-warning'>
                        Складские позиции были зарезервированы для этого заказа на производство
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        У этого заказа на производство осталась незавершенная продукция
                    </div>`;
                }
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Заказ на производство имеет незавершенную продукцию</strong><br>
                    Этот заказ на производство не может быть завершен, так как имеет незавершенный выход деталей
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                ...
                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Необходимые запасы не были полностью зарезервированы</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Требуемое количество деталей не было произведено</div>`;
                }
            }
        ...
        {},
        {
            success: function(build) {
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        BOM содержит отслеживаемые детали.<br>
                        Продукция должна создаваться индивидуально.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    ...
                    <div class='alert alert-block alert-info'>
                        ...
                        Ведите серийные номера для создания нескольких единиц продукции
                    </div>
                    `;
                }
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Зарезервировать складские позиции для этой продукции',
        );
        ...
        html += makeIconButton(
            ...
            output_id,
            'Отменить резерв этой продукции',
        );
    }
...
function deallocateStock(build_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-warning'>
    Вы уверены, что хотите отменить резерв выбранных складских позиций из этого производства?
    </dvi>
    `;
    ...
    constructForm(url, {
        ...
        title: 'Отменить резерв складских позиций',
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Выбрать Продукцию',
            'Как минимум одна единица продукции должна быть выбрана',
        );
        return;
    }
    ...
    var html = `
    <div class='alert alert-block alert-success'>
    Выбранная продукция будет отмечена как завершенная
    </div>
...
function scrapBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Выбранная продукция будет отмечена как списанная
    <ul>
        <li>Списанная продукция отмечена как отклоненная</li>
        <li>Зарезервированная складская позиция более не доступна</li>
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Выбранная продукция будет удалена
    <ul>
    <li>Продукция будет полностью удалена</li>
    <li>Зарезервированные складские позиции были возвращены на склад</li>
    </ul>
    </div>
    <table class='table table-striped table-condensed' id='build-complete-table'>
        <thead>
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    setupFilterList('builditems', $(table), options.filterTarget || '#filter-list-incompletebuilditems', {
        ...
        plural_name: 'продукция',
        custom_actions: [{
            label: 'buildoutput',
            icon: 'fa-tools',
            title: 'Действия с продукцией',
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Активная продукция не найдена';
        },
        ...
        columns: [
            ...
            },
            {
                ...
                title: 'Зарезервированные Строки',
                visible: false,
                sortable: true,
                switchable: false,
                formatter: function(value, row) {
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    html += constructField(
        ...
        {
            type: 'related field',
            label: 'Место хранения - источник',
            help_text: 'Выберите место хранения - источник (оставьте пустым, чтобы взять из всех мест)',
            required: false,
        },
        {},
    );
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                ...
                <th style='min-width: 250px;'>Складская позиция</th>
                <th>Количество</th>
                <th></th>
            </tr>
        </thead>
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Зарезервировать складские позиции для этого заказа на производства',
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'Нет совпадающих складских позиций';
                        }
                    },
                    null,
                    options,
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    Складские позиции будут автоматически зарезервированы на этот заказ на производстве, в соответствии с указанными рекомендациями:
    <ul>
    ...
    constructForm(`/api/build/${build_id}/auto-allocate/`, {
        method: 'POST',
        fields: fields,
        title: 'Зарезервировать Складские Позиции',
        confirm: true,
        preFormContent: html,
        onSuccess: function(response) {
            if (options.onSuccess) {
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Место хранения не установлено</i>';
                    }
                }
            },
            {
...
function loadBuildLineTable(table, build_id, options={}) {
    ...
    if (!options.data) {
        setupFilterList('buildlines', $(table), filterTarget, {
            ...
            singular_name: 'строка производства',
            plural_name: 'позиция производства',
        });
    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Расходник'></span>`;
                    } else {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `В заказе: ${formatDecimal(row.on_order)}`);
                    }

                    if (row.in_production && row.in_production > 0) {
                        icons += makeIconBadge('fa-tools icon-blue', `В производстве: ${formatDecimal(row.in_production)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Запасы производства');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Заказать запасы');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Зарезервировать Остатки');
                    }
