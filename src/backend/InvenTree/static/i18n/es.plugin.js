            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.custom && !row.is_builtin && row.is_installed) {
                        if (row.active) {
                            buttons += makeIconButton('fa-stop-circle icon-red', 'btn-plugin-disable', row.key, 'Desactivar Plugin');
                        } else {
                            buttons += makeIconButton('fa-play-circle icon-green', 'btn-plugin-enable', row.key, 'Activar Plugin');
                        }
                    }
...
function activatePlugin(plugin_id, active=true) {
    ...
    ¿Estás seguro de que deseas activar este plugin?
    </span>
    ` : `
    <span class='alert alert-block alert-danger'>
    ¿Estás seguro de que deseas desactivar este plugin?
    </span>
    `;

    constructForm(null, {
        title: active ? 'Activar Plugin' : 'Desactivar Plugin',
        preFormContent: html,
        confirm: true,
        submitText: active ? 'Activar' : 'Desactivar',
            ...
            inventreePut(
                ...
                },
                {
                    ...
                    success: function() {
                        ...
                        addCachedAlert('Plugin actualizado', {style: 'success'});
                        location.reload();
                    },
                    error: function(xhr) {
                        $(opts.modal).modal('hide');
