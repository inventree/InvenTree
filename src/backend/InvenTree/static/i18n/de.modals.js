    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('Ungültige Serverantwort', 'JSON Antwort enthält keine Formulardaten');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog(
                    'Fehler 401: Nicht authentifiziert',
                    'Anmeldeinformationen nicht angegeben',
                );
            } else if (xhr.status == 403) {
                showAlertDialog(
                    'Fehler 403: Zugriff verweigert',
                    'Sie haben nicht die erforderliche Berechtigung auf diesen Inhalt zuzugreifen',
                );
            } else if (xhr.status == 404) {
                showAlertDialog(
                    'Fehler 404: Ressource nicht gefunden',
                    'Die angefragte Ressource konnte auf dem Server nicht gefunden werden',
                );
            } else if (xhr.status == 408) {
                showAlertDialog(
                    'Fehler 408: Zeitüberschreitung',
                    'Zeitüberschreitung beim Abfragen der Daten vom Server',
                );
            } else {
