...
function printLabels(options) {
    ...
    if (!options.items || options.items.length == 0) {
        showAlertDialog(
            'Selecione os Itens',
            'Nenhum item selecionado para impressão',
        );
        return;
    }
    ...
    function getPrintingFields(plugin_slug, callback) {
        ...
        let url = '/api/label/print/' + `?plugin=${plugin_slug}`;
        ...
        onSuccess: function(response) {
            if (response.complete) {
                if (response.output) {
                    ...
                    showMessage('Etiquetas enviadas à impressora', {
                        style: 'success'
                    });
                }
            } else {
                showMessage('Impressão de etiqueta falhou', {
                    style: 'warning',
                });
            }
        }
