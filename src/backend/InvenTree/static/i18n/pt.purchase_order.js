...
function receivePurchaseOrderItems(order_id, line_items, options={}) {
    ...
    <table class='table table-condensed' id='order-receive-table'>
        <thead>
            <tr>
                ...
                <th>Recebido</th>
                <th style='min-width: 50px;'>Quantidade a Receber</th>
    ...
    constructForm(`/api/order/po/${order_id}/receive/`, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Confirmar o recebimento dos itens',
        title: 'Receber Itens do Pedido de Compra',
        afterRender: function(fields, opts) {
            ...
            if (global_settings.BARCODE_ENABLE) {
                $(opts.modal).find('.button-row-add-barcode').click(function() {
                    ...
                    barcodeDialog('Escanar o Código de Barras do Item', {
                        details: 'Ler código de barras no item de entrada (não deve corresponder a nenhum item de estoque existente)',
                        ...
                        onError400: function(response, barcode_options) {
                            if (response.barcode_data && response.barcode_hash) {
                                ...
                                showBarcodeMessage(barcode_options.modal, 'Dados do código de barras inválido');
                            }
                        }
                    });
                });
...
function deletePurchaseOrderLineItems(items, options={}) {
    ...
    Todos os Itens de Linha selecionadas serão excluídos
    </div>
    ...
    constructForm('/api/order/po-line/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Excluir itens de linha selecionados?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadPurchaseOrderLineItemTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'Nenhum item de linha encontrado';
        },
        queryParams: filters,
        original: options.params,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.target_date) {
                        ...
                        if (row.overdue) {
                            html += makeIconBadge('fa-calendar-times icon-red', 'Este item de linha está atrasado');
                        }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (options.allow_receive && row.received < row.quantity) {
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-line-receive', pk, 'Receber item de linha');
                    }
