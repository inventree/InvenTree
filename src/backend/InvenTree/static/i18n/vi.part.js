...
function partGroups() {
    ...
    return {
        ...
        initial_stock: {
            title: 'Số liệu kho ban đầu',
            collapsible: true,
            hidden: !global_settings.PART_CREATE_INITIAL,
        },
        initial_supplier: {
            title: 'Dữ liệu nhà cung cấp ban đầu',
            collapsible: true,
    ...
    if (part.total_in_stock) {
        ...
        if ((part.minimum_stock > 0) && (part.minimum_stock > part.total_in_stock)) {
            ...
            if (options.hideTotalStock) {
                elements.push(`Có sẵn: ${part.unallocated_stock}`);
            } else {
                elements.push(`Có sẵn: ${part.unallocated_stock}/${part.in_stock}`);
            }
        } else {
        ...
        elements.push(`Hết hàng`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`Bật đơn hàng: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Đang dựng: ${part.building}`);
    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Số lượng gói: ${pq} - Tổng số lượng: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Số lượng gói: ${pq} - Tổng số lượng: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Đã nhận</span>`;
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>Hết hàng</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>Hết hàng</span>`;
    }

    rows += `<tr><td><b>Kiện hàng</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>Bật đơn hàng</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Đang dựng</b></td><td>${part.building} ${units}</td></tr>`;
    }
