    ...
    getApiEndpointOptions(url, function(OPTIONS) {
        ...
        case 'POST':
            if (canCreate(OPTIONS)) {
                ...
                showMessage('Aktion nicht erlaubt', {
                    style: 'danger',
                    details: 'Erstellen ist nicht erlaubt',
        ...
        case 'PATCH':
            if (canChange(OPTIONS)) {
                ...
                showMessage('Aktion nicht erlaubt', {
                    style: 'danger',
                    details: 'Aktualisieren ist nicht erlaubt',
                    icon: 'fas fa-user-times',
                });
...
function initializeRelatedField(field, fields, options={}) {
    ...
    if(field.tree_picker) {
        ...
        button.on("click", () => {
            ...
            const title = 'Auswählen' + " " + options.actions[name].label;
            const content = `
                <div class="mb-1">
                    <div class="input-group mb-2">
                        <input class="form-control" type="text" id="${name}_tree_search" placeholder="Suche ${options.actions[name].label}..." />
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <button class='input-group-text form-clear' id='clear_${field_name}' title='Eingabe löschen'>
