    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.is_customer) {
                    html += `<span title='Kund' class='fas fa-user-tie float-right'></span>`;
                }

                if (row.is_manufacturer) {
                    html += `<span title='Tillverkare' class='fas fa-industry float-right'></span>`;
                }

                if (row.is_supplier) {
                    html += `<span title='Leverantör' class='fas fa-building float-right'></span>`;
                }
...
function loadContactTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        sidePagination: 'server',
        formatNoMatches: function() {
            return 'Inga kontakter hittades';
        },
        showColumns: true,
        name: 'contacts',
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('btn-contact-edit', pk, 'Redigera kontakt');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('btn-contact-delete', pk, 'Radera kontakt');
                    }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('btn-address-edit', pk, 'Redigera adress');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('btn-address-delete', pk, 'Radera adress');
                    }
