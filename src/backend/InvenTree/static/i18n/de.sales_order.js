    ...
    inventreeGet(`/api/order/so/shipment/${shipment_id}/`, {}, {
        success: function(shipment) {
            ...
            if (!allocations || allocations.length == 0) {
                ...
                Dieser Sendung wurden keine Lagerartikel zugewiesen
                </div>
                `;
            } else {
                html = `
                Die folgenden Lagerartikel werden verschickt
            ...
            constructForm(`/api/order/so/shipment/${shipment_id}/ship/`, {
                method: 'POST',
                title: `Lieferung fertigstellen ${shipment.reference}`,
                fields: {
                    shipment_date: {
                ...
                preFormContent: html,
                confirm: true,
                confirmMessage: 'Lieferung bestätigen',
...
function completePendingShipments(order_id, options={}) {
    ...
    if (allocated_shipments.length > 0) {
        ...
        if (!pending_shipments.length) {
            ...
            html += `
            Keine Lagerartikel wurden offenen Sendungen zugewiesen
            `;
        }
        ...
        `/api/order/so/${order_id}/complete/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                if (!opts.context.is_complete) {
                    ...
                    Versenden dieses Auftrags bedeutet, dass der Auftrag und seine Positionen nicht mehr bearbeitbar sind.
                    </div>`;
                }
    ...
    constructForm(
        `/api/order/so/${order_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                var html = `
                <div class='alert alert-block alert-warning'>
                Stornieren dieser Bestellung bedeutet, dass sie nicht länger bearbeitbar ist.
                </div>`;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Bestellung ist überfällig');
                    }
...
function loadSalesOrderShipmentTable(table, options={}) {
    ...
    function makeShipmentActions(row) {
        ...
        html += makeEditButton('button-shipment-edit', pk, 'Lieferung bearbeiten');

        if (!options.shipped) {
            html += makeIconButton('fa-truck icon-green', 'button-shipment-ship', pk, 'Lieferung fertigstellen');
        }

        var enable_delete = row.allocations && row.allocations.length == 0;

        html += makeDeleteButton('button-shipment-delete', pk, 'Lieferung löschen', {disabled: !enable_delete});
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine passende Lieferung gefunden';
        },
        columns: [
            {
                visible: false,
...
function allocateStockToSalesOrder(order_id, line_items, options={}) {
    ...
    constructForm(`/api/order/so/${order_id}/allocate/`, {
        ...
        title: 'Lagerartikel Auftrag zuweisen',
...
function loadSalesOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        original: options.params,
        formatNoMatches: function() {
            return 'Keine Allokationen für Auftrag gefunden';
        },
        columns: [
            {
                field: 'pk',
        ...
        table.find('.button-allocation-delete').click(function() {
            ...
            constructForm(
                `/api/order/so-allocation/${pk}/`,
                {
                    ...
                    title: 'Bestandszuordnung löschen',
                    refreshTable: options.table,
                }
            );
        });
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        return `<em>An den Kunden versandt - ${row.shipment_date}</em>`;
                        ...
                        return `<em>Lagerort nicht angegeben</em>`;
                    }
                },
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (available > 0) {
                        ...
                        if (row.available_variant_stock && row.available_variant_stock > 0) {
                            html += makeIconBadge('fa-info-circle icon-blue', 'Alternatives Lager einschließen');
                        }
                    } else {
                    ...
                    if (required > 0) {
                        if (available >= required) {
                            html += makeIconBadge('fa-check-circle icon-green', 'Ausreichender Bestand vorhanden');
                        } else {
