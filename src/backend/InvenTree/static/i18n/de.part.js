...
function partGroups() {
    ...
    return {
        ...
        duplicate: {
            title: 'Duplizierungsoptionen für Teile',
            collapsible: true,
        },
        initial_stock: {
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        ...
        follow: true,
        persist: true,
        persistMessage: 'Neue Kategorie nach dieser Kategorie erstellen',
        successMessage: 'Artikelkategorie erstellt'
    });
}
...
function editCategory(pk) {
    ...
    constructForm(url, {
        fields: fields,
        title: 'Teilekategorie bearbeiten',
        reload: true,
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    Möchten Sie diese Artikel-Kategorie wirklich löschen?
    </div>`;
    var subChoices = [
        {
            value: 0,
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        ...
        persist: true,
        persistMessage: 'Weiteres Teil nach diesem erstellen',
...
function deletePart(pk, options={}) {
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            ...
            <li>Alle Hersteller- und Lieferanteninformationen für dieses Teil werden gelöscht</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'Du hast Benachrichtigungen zu diesem Artikel abonniert');

                            showMessage('Du hast Benachrichtigungen zu diesem Artikel abonniert', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Benachrichtigungen zu diesem Artikel abonnieren');

                            showMessage('Du erhältst keine Benachrichtigungen zu diesem Artikel mehr', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Die Stückliste zu validieren markiert jede Position als gültig
    </div>
    `;
...
function makePartIcons(part) {
    ...
    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Abonniertes Teil');
    }
...
function generateStocktakeReport(options={}) {
    ...
    Erstellung eines neuen Inventurberichts planen. Nach Fertigstellung steht der Inventurbericht zum Download zur Verfügung.
    </div>
    `;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Inventurbericht erstellen',
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Alternatives Lager einschließen' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Keine Teileparametervorlagen gefunden';
        },
        columns: [
            {
                field: 'pk',
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Teileparametervorlage bearbeiten',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
            Alle Parameter mit Verweis auf diese Vorlage werden ebenfalls gelöscht
        </div>`;
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Teileparametervorlage löschen',
                refreshTable: table,
            }
        );
    });
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Packmenge: ${pq} - Gesamtstückzahl: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let pq = row.supplier_part_detail.pack_quantity_native;
                        let total = value * pq;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Packmenge: ${pq} - Gesamtstückzahl: ${total}'></span>`;
                    }
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Teilebeziehung löschen');
...
function setPartCategory(data, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        Wähle die Kategorie für die ausgewählten Teile
    </div>
    `;

    constructForm('/api/part/change_category/',{
        title: 'Teilekategorie auswählen',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Keine passenden Testvorlagen gefunden';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.results && row.results > 0) {
                        html += `
                        <span class='badge bg-dark rounded-pill float-right' title='${row.results} Ergebnisse'>
                            ${row.results}
                        </span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        var text = 'Dieser Test ist für ein übergeordnetes Teil definiert';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Vorlage für Testergebnis bearbeiten',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Vorlage für Testergebnis löschen',
                    refreshTable: table,
                });
            });
        }
