...
function renderStockItem(data, parameters={}) {
    ...
    if (data.quantity == 0) {
        stock_detail = `<span class='badge rounded-pill bg-danger'>No Stock</span>`;
    } else {
        if (data.serial && data.quantity == 1) {
            ...
            if (render_available_quantity) {
                ...
                stock_detail = `Dostupné: ${available}`;
            } else {
                stock_detail = `Množství: ${data.quantity}`;
            }
        }

        if (data.batch) {
            stock_detail += ` - <small>Šarže: ${data.batch}</small>`;
        }
    }
...
function renderPart(data, parameters={}) {
    ...
    if (user_settings.PART_SHOW_QUANTITY_IN_FORMS) {
        ...
        if (!data.active) {
            labels += `<span class='badge badge-right rounded-pill bg-danger'>Neaktivní</span>`;
        }
    }
    ...
    return renderModel(
        {
            ...
            textSecondary: `Doprava ${data.reference}`,
        },
        parameters
    );
}
