...
function generateAvailableFilterList(tableKey) {
    ...
    html += `<option value=''>Selecionar filtro</option>`;
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        let print_buttons = `
        <div class='btn-group' role='group'>
        <button id='printing-options' title='Ações de Impressão' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
        ...
        if (report_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-report-${tableKey}'><span class='fas fa-file-pdf'></span> Imprimir Relatórios</a></li>`;
        }
    ...
    element.find(`#${add}`).click(function clicked() {
        ...
        if (!addClicked) {
            ...
            html += `<button title='Criar filtro' class='btn btn-outline-secondary filter-button' id='${make}'><span class='fas fa-plus'></span></button>`;
            html += `</div>`;
