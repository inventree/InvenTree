...
function constructOrderTableButtons(options={}) {
    ...
    if (!options.disableCalendarView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_calendar}' title='显示日历视图'><span class='fas fa-calendar-alt'></span></button>`,
            event: function() {
    ...
    if (!options.disableListView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_list}' title='显示列表视图'><span class='fas fa-th-list'></span></button>`,
            event: function() {
                buttonCallback('list');
            }
        });
    ...
    if (!options.disableTreeView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_tree}' title='显示树状视图'><span class='fas fa-sitemap'></span></button>`,
            event: function() {
                buttonCallback('tree');
            }
        });
...
function constructExpandCollapseButtons(table, idx=0) {
    ...
    return [
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='展开所有行'><span class='fas fa-expand'></span></button>`,
            event: function() {
                $(table).bootstrapTable('expandAllRows');
            }
        },
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='折叠所有行'><span class='fas fa-compress'></span></button>`,
            event: function() {
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatShowingRows: function(pageFrom, pageTo, totalRows) {
            ...
            if (totalRows === undefined || isNaN(totalRows)) {
                ...
                return `显示 ${pageFrom} 至 ${pageTo} 的 ${totalRows} 行`;
            }
        },
