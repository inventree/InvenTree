...
function generateAvailableFilterList(tableKey) {
    ...
    html += `<option value=''>选择筛选器</option>`;
...
function makeCustomActionGroup(action_group, table) {
    ...
    let title = action_group.title || '动作';
...
function makeBarcodeActions(barcode_actions, table) {
    ...
    let html = `
    <div class='btn-group' role='group'>
    <button id='barcode-actions' title='条形码操作' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
...
function setupFilterList(tableKey, table, target, options={}) {
    ...
    if (report_button || labels_button) {
        let print_buttons = `
        <div class='btn-group' role='group'>
        <button id='printing-options' title='打印操作' class='btn btn-outline-secondary dropdown-toggle' type='button' data-bs-toggle='dropdown'>
        ...
        if (report_button) {
            print_buttons += `<li><a class='dropdown-item' href='#' id='print-report-${tableKey}'><span class='fas fa-file-pdf'></span> 打印报告</a></li>`;
        }
    ...
    element.find(`#${add}`).click(function clicked() {
        ...
        if (!addClicked) {
            ...
            html += `<button title='创建筛选条件' class='btn btn-outline-secondary filter-button' id='${make}'><span class='fas fa-plus'></span></button>`;
            html += `</div>`;
