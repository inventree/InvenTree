        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Objednávka sestavy má neúplné výstupy</strong><br>
                ...
                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Požadované zásoby nebyly plně přiděleny</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Požadované množství sestavy nebylo dokončeno</div>`;
                }
            }
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        ...
        output_html += `Množství: ${output.quantity}`;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Šarže: ${row.batch})</small>`;
                    }
    ...
    <table class='table table-striped table-condensed' id='stock-allocation-table'>
        <thead>
            <tr>
                <th>Díl</th>
                <th>Přiděleno</th>
                <th style='min-width: 250px;'>Skladové položky</th>
                <th>Množství</th>
                <th></th>
            </tr>
        </thead>
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `Na objednávku: ${formatDecimal(row.on_order)}`);
                    }

                    if (row.in_production && row.in_production > 0) {
                        icons += makeIconBadge('fa-tools icon-blue', `Ve výrobě: ${formatDecimal(row.in_production)}`);
                    }
