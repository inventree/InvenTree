        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    if (enableDelete) {
                        bDel = `<button title='删除通知' class='notification-delete btn btn-outline-secondary' type='button' pk='${row.pk}'><span class='fas fa-trash-alt icon-red'></span></button>`;
                    }
        ...
        },
        {
            success: function(response) {
                if (response.length == 0) {
                    html = `<p class='text-muted'><em>无未读通知</em><span class='fas fa-check-circle icon-green float-right'></span></p>`;
                } else {
                    // build up items
...
function closeNotificationPanel() {
    $('#notification-center').html(`<p class='text-muted'>通知将在此处加载</p>`);
}
