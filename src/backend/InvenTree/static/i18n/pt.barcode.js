...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || 'Leia o código de barras aqui usando um leitor de código de barras';

    hintText = hintText || 'Digitar informações do código de barras';
    ...
    <div class='form-group'>
        ...
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Ler código de barras usando webcam conectada' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Digite notas opcionais para transferência de estoque';
    var placeholder = options.placeholder || 'Inserir anotações';
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Resposta do servidor inválida<br>Situação: '${status}'`
    );
}
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || 'Ler dados do código de barras';
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Ler Código de Barras';
...
function unlinkBarcode(data, options={}) {
    ...
    html += 'Isto irá remover o link com o código de barras associado';

    showQuestionDialog(
        'Desatribuir Código de Barras',
        html,
        {
            accept_text: 'Desassociar',
            accept: function() {
                inventreePut(
                    '/api/barcode/unlink/',
                    data,
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Remover item de estoque')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Escanear Itens de Estoque para Local',
        {
            details: 'Digitalize o código de barras dos itens de estoque para fazer check-in nesta localização',
            headerContent: table,
            preShow: function() {
            ...
            onSubmit: function() {
                ...
                if (entries.length == 0) {
                    showBarcodeMessage(modal, 'Nenhum código de barras fornecido', 'warning');
                    return;
                }
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Item de estoque já escaneado', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Item de estoque já está nesta localização');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Item de estoque adicionado', 'success');
                    ...
                    showBarcodeMessage(modal, 'Código de barras não corresponde a item de estoque válido', 'warning');
                }
            },
        }
    );
...
function barcodeCheckInStockLocations(location_id, options={}) {
    ...
    barcodeDialog(
        'Escanear o Recipiente de Estoque em Local',
        {
            details: 'Digitalize o código de barras do contêiner para fazer check-in para esta localização',
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, 'Código de barras não corresponde ao local de estoque válido', 'warning');
                }
            }
        }
    );
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'Código de barras não corresponde a um local válido',
                                'warning',
                            );
                        }
                    });
