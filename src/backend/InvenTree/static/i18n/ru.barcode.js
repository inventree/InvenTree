...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Добавьте записи для перемещения запасов (необязательно)';
    var placeholder = options.placeholder || 'Введите записи';

    return `
    <div class='form-group'>
        <label class='control-label' for='notes'>Записи</label>
        <div class='controls'>
            <div class='input-group'>
                <span class='input-group-text'>
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Сканировать штрихкод';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Отвязать штрих-код</b><br>`;
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Удалить складскую позицию')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        'Сканировать складские позиции в место хранения',
        {
            details: 'Сканировать штрих-код складской позиции и проверить в этом месте хранения',
            headerContent: table,
            preShow: function() {
                modalSetSubmitText(modal, 'Отметить');
                modalEnable(modal, false);
                reloadTable();
            },
            onShow: function() {
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                            ...
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    ...
                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Складская позиция уже в этом месте хранения');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Добавленная складская позиция', 'success');
                    ...
                    showBarcodeMessage(modal, 'Штрих-код не совпадает с допустимыми складскими позициями', 'warning');
                }
            },
        }
    );
