...
function serializeStockItem(pk, options={}) {
    ...
    options.method = 'POST';
    options.title = 'Сериализировать складскую позицию';
...
function deleteStockLocation(pk, options={}) {
    ...
    constructForm(url, {
        ...
        fields: {
            'delete_stock_items': {
                label: 'Действия со складской позиции в этом месте хранения',
                choices: subChoices,
                type: 'choice'
            },
...
function stockItemFields(options={}) {
    var fields = {
        ...
        quantity: {
            help_text: 'Введите начальное количество для этой складской позиции',
        },
        serial_numbers: {
            icon: 'fa-hashtag',
            type: 'string',
...
function duplicateStockItem(pk, options) {
    ...
    if (!options.onSuccess) {
        options.onSuccess = function(response) {
            ...
            showAlertOrCache('Складская позиция дублирована', true, {style: 'success'});
    ...
    inventreeGet(`/api/stock/${pk}/`, {}, {
        success: function(data) {
            ...
            options.method = 'POST';
            options.title = 'Дублировать складскую позицию';
...
function deleteStockItem(pk, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Вы уверены, что хотите удалить эту складскую позицию?
    </div>`;

    constructForm(url, {
        method: 'DELETE',
        title: 'Удалить складскую позицию',
        preFormContent: html,
        onSuccess: function(response) {
            handleFormSuccess(response, options);
        }
...
function editStockItem(pk, options={}) {
    ...
    options.title = 'Редактировать складскую позицию';
...
function createNewStockItem(options={}) {
    ...
    options.title = 'Новая складская позиция';
    options.method = 'POST';

    options.create = true;

    options.persist = true;
    options.persistMessage = 'Создать еще один элемент после этого';
    options.successMessage = 'Складская позиция создана';
...
function assignStockToCustomer(items, options={}) {
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.serial && item.quantity == 1) {
            quantity = `Серийный номер: ${item.serial}`;
        } else {
...
function mergeStockItems(items, options={}) {
    ...
    <strong>Некоторые данные будут потеряны при слиянии складских позиций:</strong>
    <ul>
    ...
    constructForm('/api/stock/merge/', {
        ...
        confirm: true,
        confirmMessage: 'Подтвердить слияние складских позиций',
        title: 'Объединить складские позиции',
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
                var pk = $(this).attr('pk');
...
function adjustStock(action, items, options={}) {
    ...
    case 'move':
        ...
        break;
    case 'count':
        formTitle = 'Установить запасы';
        actionTitle = 'Количество';
        url = '/api/stock/count/';
        break;
    case 'take':
        formTitle = 'Удалить запасы';
        actionTitle = 'Взять';
        url = '/api/stock/remove/';
        break;
    case 'add':
        formTitle = 'Добавить Запасы';
    ...
    for (var idx = 0; idx < items.length; idx++) {
        ...
        if (item.batch) {
            quantityString += ` - <small>Партия: ${item.batch}</small>`;
        }
        ...
        const batch_input = constructField(
            `items_batch_code_${pk}`,
            {
                ...
                required: false,
                label: 'Код партии',
                help_text: 'Введите код партии для поступающих складских позиций',
                icon: 'fa-layer-group',
                value: item.batch,
            },
            {
    ...
    if (itemCount == 0) {
        showAlertDialog(
            'Выбрать складские позиции',
            'Выбрать как минимум одну складскую позицию',
        );
    ...
    constructForm(url, {
        ...
        preFormContent: html,
        confirm: true,
        confirmMessage: 'Подтвердите изменение запасов',
        title: formTitle,
        afterRender: function(fields, opts) {
            // Add button callbacks to remove rows
            $(opts.modal).find('.button-stock-item-remove').click(function() {
...
function passFailBadge(result) {
    ...
    if (result) {
        return `<span class='badge badge-right rounded-pill bg-success'>ПРОШЕЛ</span>`;
    } else {
        return `<span class='badge badge-right rounded-pill bg-danger'>ПРОВАЛЕН</span>`;
    }
}

function noResultBadge() {
    return `<span class='badge badge-right rounded-pill bg-info'>НЕТ РЕЗУЛЬТАТА</span>`;
}
...
function loadStockTestResultsTable(table, options) {
    ...
    function makeButtons(row, grouped) {
        ...
        if (row.requires_attachment == false && row.requires_value == false && !row.result) {
            ...
            html += makeIconButton('fa-check-circle icon-green', 'button-test-tick', row.test_name, 'Тест пройден');
        }

        html += makeIconButton('fa-plus icon-green', 'button-test-add', row.templateId, 'Добавить Результат Тестирования');

        if (!grouped && row.result != null) {
            html += makeEditButton('button-test-edit', row.testId, 'Редактировать результаты тестирования');
    ...
    $(table).on('click', '.button-test-add', function() {
        ...
        constructForm('/api/stock/test/', {
            method: 'POST',
            fields: fields,
            title: 'Добавить Результат Тестирования',
...
function locationDetail(row, showLink=true) {
    ...
    if (row.consumed_by) {
        text = 'Поглощен заказом на производство';
        ...
        text = `Установленные складские позиции ${row.belongs_to}`;
        ...
        text = '<i>Место хранения не установлено</i>';
        url = '';
    }
            ...
            callback: function(data) {
                mergeStockItems(data, {
                    success: function(response) {
                        ...
                        showMessage('Объединенные складские позиции', {
                            style: 'success',
                        });
                    }
                });
...
function loadStockTable(table, options) {
    ...
    col = {
        ...
        formatter: function(value, row) {
            ...
            if (row.is_building) {
                html += makeIconBadge('fa-tools', 'Складская позиция в производстве');
            }

            if (row.sales_order) {
                // Stock item has been assigned to a sales order
                html += makeIconBadge('fa-truck', 'Складская позиция зарезервирована заказом на продажу');
            } else if (row.customer) {
                // StockItem has been assigned to a customer
                html += makeIconBadge('fa-user', 'Складская позиция была назначена покупателю');
            } else if (row.allocated) {
                if (row.serial != null && row.quantity == 1) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'Сериализированная складская позиция была зарезервирована ');
                } else if (row.allocated >= row.quantity) {
                    html += makeIconBadge('fa-bookmark icon-yellow', 'Складские позиции были полностью зарезервированы');
                } else {
                    html += makeIconBadge('fa-bookmark', 'Складские позиции были частично зарезервированы');
                }
            } else if (row.belongs_to) {
                html += makeIconBadge('fa-box', 'Складская позиция была установлена в другую деталь');
            } else if (row.consumed_by) {
                html += makeIconBadge('fa-tools', 'Складская позиция была поглощена заказом на продажу');
            }

            if (row.expired) {
                html += makeIconBadge('fa-calendar-times icon-red', 'Складская позиция была просрочена');
            } else if (row.stale) {
                html += makeIconBadge('fa-stopwatch', 'Складская позиция будет просрочена в скором времени');
            }

            // Special stock status codes
            if (row.status == stockCodes.REJECTED.key) {
                html += makeIconBadge('fa-times-circle icon-red', 'Складская позиция была отклонена');
            } else if (row.status == stockCodes.LOST.key) {
                html += makeIconBadge('fa-question-circle', 'Складская позиция была утеряна');
            } else if (row.status == stockCodes.DESTROYED.key) {
                html += makeIconBadge('fa-skull-crossbones', 'Складская позиция была уничтожена');
            }

            if (row.quantity <= 0) {
                html += `<span class='badge badge-right rounded-pill bg-danger'>Истощен</span>`;
            }
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Нет складских позиций соответствующих запросу';
        },
...
function loadStockLocationTable(table, options) {
    ...
    setupFilterList(filterKey, table, filterListElement, {
        ...
        singular_name: 'места хранения',
        plural_name: 'места хранения',
    });
    ...
    cols.push({
        ...
        formatter: function(details, row) {
            ...
            if (!details || !Object.keys(details).length) {
                return `<small><em>Нет изменений</em></small>`;
            }
            ...
            if (details.buildorder) {
                html += `<tr><th>Заказ на производство</th>`;
                html += `<td>`;
            ...
            if (details.salesorder) {
                html += `<tr><th>Заказ на продажу</th>`;
                html += '<td>';
            ...
            if (details.returnorder) {
                html += `<tr><th>Заказ на возврат</th>`;
                html += '<td>';
            ...
            if (details.stockitem) {
                html += '<tr><th>Складская позиция</td>';
                ...
                if (details.stockitem_detail) {
                    ...
                    html += `<i>Складская позиция не существует</i>`;
                }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.serial && row.quantity == 1) {
                        html += `Серийный номер: ${row.serial}`;
                    } else {
                        html += `${row.quantity}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.can_edit) {
                        html += makeIconButton('fa-unlink', 'button-uninstall', pk, 'Снять складскую позицию');
                    }
        ...
        `/api/stock/${installed_item_id}/uninstall/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                if (installed_item_id == null) {
                    ...
                    Выберите складскую позицию для съема
                    </div>`;
                }
...
function installStockItem(stock_item_id, part_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        <strong>Установить другую складскую позицию в эту деталь</strong><br>
        Складские позиции могут быть установлены, только если отвечают следующим критериям:<br>
        <ul>
            <li>Складская позиция ссылается на деталь, чья спецификация является этой складской позицией</li>
            <li>Складская позиция сейчас доступна на складе</li>
            <li>Складская позиция не установлена в другую деталь</li>
            <li>Складская позиция отслеживается либо по коду партии, либо серийному номеру</li>
        </ul>
    </div>`;
    ...
    constructForm(
        `/api/stock/${stock_item_id}/install/`,
        {
            ...
            confirm: true,
            title: 'Установить складскую позицию',
            preFormContent: html,
            onSuccess: function(response) {
                if (options.onSuccess) {
...
function setStockStatus(items, options={}) {
    ...
    if (items.length == 0) {
        showAlertDialog(
            'Выбрать складские позиции',
            'Выберите одну или более складских позиций'
        );
        return;
    }
    ...
    Выбранные складские позиции: ${items.length}
    </div>`;
