...
function initializeRelatedField(field, fields, options={}) {
    ...
    if(field.tree_picker) {
        ...
        button.on("click", () => {
            ...
            const title = 'Выбрать' + " " + options.actions[name].label;
            const content = `
                <div class="mb-1">
                    <div class="input-group mb-2">
                        <input class="form-control" type="text" id="${name}_tree_search" placeholder="Поиск ${options.actions[name].label}..." />
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <button class='input-group-text form-clear' id='clear_${field_name}' title='Очистить ввод'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>Столбец Файла</th><th>Имя Поля</th></tr>`;
