...
function editBuildOrder(pk) {
    ...
    constructForm(`/api/build/${pk}/`, {
        fields: fields,
        reload: true,
        title: 'Editar Pedido de Produção',
    });
}
        ...
        `/api/build/${build_id}/cancel/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                <div class='alert alert-block alert-info'>
                    Tem certeza que deseja cancelar essa produção?
                </div>`;

                if (opts.context.has_allocated_stock) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Itens de estoque foram alocados para este pedido de produção
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        Há saídas incompletas restantes para este pedido de produção
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                Pedido de produção está pronto para ser concluído'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    Este pedido de produção não pode ser concluído pois há saídas incompletas
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Pedido de Produção está incompleto</strong>
                    </div>
                    `;
                }
        ...
        reload: true,
        confirm: true,
        title: 'Completar Pedido de Produção',
        method: 'POST',
    });
}
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Próximo número de série disponível: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Último número de série: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        A Lista de Materiais (BOM) contém peças rastreáveis.<br>
                        Saída de produção deve ser gerada individualmente.
                    </div>
                    `;
                }
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Peças rastreáveis podem ter números de séries especificados<br>
                        Digite números de série para gerar várias saídas de produção simples
                    </div>
                    `;
                }
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Alocar itens de estoque para a saída de produção',
        );
        ...
        html += makeIconButton(
            ...
            output_id,
            'Desalocar estoque da saída de produção',
        );
    }
...
function deallocateStock(build_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-warning'>
    Tem certeza que deseja desalocar os itens de estoque selecionados desta produção?
    </dvi>
    `;
    ...
    constructForm(url, {
        ...
        title: 'Desalocar Items de Estoque',
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            'Selecionar Saída de Produção',
            'Ao menos uma saída de produção deve ser selecionada',
        );
        return;
    }
    ...
    var html = `
    <div class='alert alert-block alert-success'>
    Saídas de produção selecionadas serão marcadas como completas
    </div>
    ...
    constructForm(`/api/build/${build_id}/complete/`, {
        ...
        confirm: true,
        title: 'Concluir Saídas de Produção',
...
function scrapBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Saídas de produção selecionadas serão marcadas como sucatas
    <ul>
        <li>Saídas sucateadas são marcadas como rejeitada</li>
        <li>Itens de estoque alocados não estarão mais disponíveis</li>
        <li>O estado de conclusão do pedido de produção não será ajustado</li>
    </ul>
    </div>
    ...
    constructForm(`/api/build/${build_id}/scrap-outputs/`, {
        ...
        confirm: true,
        title: 'Sucatear Saídas de Produção',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                let pk = $(this).attr('pk');
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Saídas de produção selecionadas serão apagadas
    <ul>
    <li>Dados da saída de produção serão excluídos permanentemente</li>
    <li>Itens de estoque alocados serão retornados ao estoque</li>
    </ul>
    </div>
    <table class='table table-striped table-condensed' id='build-complete-table'>
        <thead>
    ...
    constructForm(`/api/build/${build_id}/delete-outputs/`, {
        ...
        fields: {},
        confirm: true,
        title: 'Deletar Saída de Produção',
        afterRender: function(fields, opts) {
            // Setup callbacks to remove outputs
            $(opts.modal).find('.button-row-remove').click(function() {
                var pk = $(this).attr('pk');
                ...
                actions: [{
                    ...
                    callback: function(data) {
                        constructForm('/api/build/item/', {
                            method: 'DELETE',
                            multi_delete: true,
                            title: 'Delete Stock Allocations',
                            form_data: {
    ...
    $(table).inventreeTable({
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (value) {
                        return renderLink(`Stock item: ${value}`, `/stock/item/${value}/`);
                    }
                }
            },
            {
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    buttons += makeEditButton('build-item-edit', row.pk, 'Edit build allocation');
                    buttons += makeDeleteButton('build-item-delete', row.pk, 'Delete build allocation');
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhuma alocação de pedido de produção encontrada';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    setupFilterList('builditems', $(table), options.filterTarget || '#filter-list-incompletebuilditems', {
        ...
        singular_name: 'saída da produção',
        plural_name: 'saídas da produção',
        custom_actions: [{
            label: 'buildoutput',
            icon: 'fa-tools',
            title: 'Ações da saída de produção',
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'Nenhuma saída de produção ativa encontrada';
        },
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    if (line_items.length == 0) {
        ...
        showAlertDialog(
            'Selecionar Peças',
            'Você deve selecionar ao menos uma peça para alocar',
        );
    ...
    function renderBuildLineRow(build_line, quantity) {
        ...
        var quantity_input = constructField(
            `items_quantity_${pk}`,
            {
                ...
                min_value: 0,
                value: quantity || 0,
                title: 'Especifique a quantidade de alocação de estoque',
                required: true,
            },
            {
                hideLabels: true,
    ...
    if (table_entries.length == 0) {
        ...
        showAlertDialog(
            ...
            'Todas as peças selecionadas foram completamente alocadas',
        );
    ...
    html += constructField(
        'take_from',
        {
            ...
            help_text: 'Selecione o local de origem (deixe em branco para tirar de todos os locais)',
            required: false,
        },
        {},
    );
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Alocar Itens de Estoque para o Pedido de Produção',
        afterRender: function(fields, options) {
            ...
            var take_from_field = {
                ...
                noResults: function(query) {
                    return 'Nenhum local de estoque correspondente';
                },
            };
                ...
                initializeRelatedField(
                    {
                        ...
                        noResults: function(query) {
                            return 'Nenhum item de estoque correspondente';
                        }
                    },
                    null,
                    options,
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
    <strong>Alocação Automática de Estoque</strong><br>
    Itens de estoque serão automaticamente alocados para este pedido de produção, conforme as diretrizes fornecidas:
    <ul>
        <li>Se um local for especificado o estoque será apenas alocado deste local</li>
        <li>Se o estoque é considerado intercambiável será alocado a partir da primeira localização encontrada</li>
        <li>Se estoque substituto é permitido será utilizado quando o estoque primário não for encontrado</li>
    </ul>
    </div>
    `;
...
function loadBuildTable(table, options) {
    ...
    $(table).inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'Nenhuma produção corresponde a consulta';
        },
        url: '/api/build/',
        queryParams: filters,
        groupBy: false,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Pedido de produção está atrasada');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (value) {
                        ...
                        return `<i>Sem informações de usuário</i>`;
                    }
                }
            },
            {
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    if (row.location_detail) {
                        ...
                        return '<i>Nenhum local definido</i>';
                    }
                }
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Editar alocação de estoque');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Excluir alocação de estoque');
                    return wrapButtons(buttons);
                }
            }
        ]
...
function loadBuildLineTable(table, build_id, options={}) {
    ...
    if (!options.data) {
        setupFilterList('buildlines', $(table), filterTarget, {
            ...
            singular_name: 'linha de produção',
            plural_name: 'linhas de produção',
        });
    }
    ...
    let table_options = {
        ...
        formatNoMatches: function() {
            return 'Nenhuma linha produção encontrada';
        },
        columns: [
            {
                checkbox: true,
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Estoque de variantes permitido');
                    }

                    if (row.part_detail.trackable) {
                        html += makeIconBadge('fa-directions', 'Peça rastreável');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Itens consumíveis'></span>`;
                    } else {
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Estoque insuficiente disponível');
                        } else {
                            icons += makeIconBadge('fa-check-circle icon-green', 'Estoque suficiente disponível');
                        }

                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>Nenhum Estoque Disponível</span>`;
                        } else {
                            let extra = '';
                            if ((row.available_substitute_stock > 0) && (row.available_variant_stock > 0)) {
                                extra = 'Incluir estoque de variantes e substitutos';
                            } else if (row.available_variant_stock > 0) {
                                extra = 'Incluir estoque de variantes';
                            } else if (row.available_substitute_stock > 0) {
                                extra = 'Incluir estoque de substitutos';
                            }
                    ...
                    if (row.in_production && row.in_production > 0) {
                        icons += makeIconBadge('fa-tools icon-blue', `Em Produção: ${formatDecimal(row.in_production)}`);
                    }

                    if (row.external_stock > 0) {
                        icons += makeIconBadge('fa-sitemap', `External stock: ${row.external_stock}`);
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part_detail.trackable && !options.output) {
                        ...
                        return `
                        <div>
                            <em>Item rastreado</em>
                            <span title='Allocate tracked items against individual build outputs' class='fas fa-info-circle icon-blue' />
                        </div>`;
                    }
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Estoque de produção');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Pedir Estoque');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Alocar Estoque');
                    }

                    if (row.allocated > 0) {
                        buttons += makeRemoveButton('button-unallocate', pk, 'Remover alocação de estoque');
                    }
