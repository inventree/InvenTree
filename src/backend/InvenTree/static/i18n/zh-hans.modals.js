...
function createNewModal(options={}) {
    ...
    modalSetTitle(modal_name, options.title || '表单标题');
...
function loadingMessageContent() {
    ...
    return `<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> 等待服务器...`;
}
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">显示错误信息</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || '接受';
    ...
    modalSubmit(modal, function() {
        $(modal).find('.js-modal-form').ajaxSubmit({
                ...
                if ('form_valid' in response) {
                    ...
                    if (response.form_valid) {
                        ...
                        if (response.html_form) {
                            ...
                            $(modal).modal('hide');
                            showAlertDialog('来自服务器的响应无效', '服务器响应中缺少表单数据');
                        }
                    }
                } else {
                    $(modal).modal('hide');
            ...
            error: function(xhr) {
                ...
                $(modal).modal('hide');
                showAlertDialog('发布表单数据时出错', renderErrorMessage(xhr));
            },
            complete: function() {
                // TODO
            }
    ...
    var ajax_data = {
        ...
        success: function(response) {
            ...
            if (response.html_form) {
                ...
                $(modal).modal('hide');
                showAlertDialog('服务器响应无效', 'JSON 响应缺少表单数据');
            }
        },
        error: function(xhr) {
            ...
            if (xhr.status == 0) {
                ...
                showAlertDialog('请求表单数据时出错', renderErrorMessage(xhr));
            }
