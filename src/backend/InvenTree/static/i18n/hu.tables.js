...
function constructOrderTableButtons(options={}) {
    ...
    if (!options.disableCalendarView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_calendar}' title='Naptár nézet megjelenítése'><span class='fas fa-calendar-alt'></span></button>`,
            event: function() {
    ...
    if (!options.disableListView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_list}' title='Lista nézet megjenítése'><span class='fas fa-th-list'></span></button>`,
            event: function() {
                buttonCallback('list');
            }
        });
    ...
    if (!options.disableTreeView) {
        buttons.push({
            html: `<button type='button' name='${idx++}' class='btn ${class_tree}' title='Fa nézet megjelenítése'><span class='fas fa-sitemap'></span></button>`,
            event: function() {
                buttonCallback('tree');
            }
        });
...
function constructExpandCollapseButtons(table, idx=0) {
    ...
    return [
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Sorok kinyitása'><span class='fas fa-expand'></span></button>`,
            event: function() {
                $(table).bootstrapTable('expandAllRows');
            }
        },
        {
            html: `<button type='button' name='${idx++}' class='btn btn-outline-secondary' title='Sorok becsukása'><span class='fas fa-compress'></span></button>`,
            event: function() {
...
function downloadTableData(table, opts={}) {
    ...
    constructFormBody({}, {
        title: opts.title || 'Táblázat exportálása',
        fields: {
            format: {
                label: 'Formátum',
...
(function($) {
    ...
    $.fn.bootstrapTable.locales['en-US-custom'] = {
        ...
        formatRecordsPerPage: function(pageNumber) {
            return `${pageNumber} sor oldalanként`;
        },
        formatShowingRows: function(pageFrom, pageTo, totalRows) {

            if (totalRows === undefined || isNaN(totalRows)) {
                return 'Összes sor mutatása';
            } else {
                return `Látható ${pageFrom} - ${pageTo} a ${totalRows} sorból,`;
            }
        },
