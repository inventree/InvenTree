        ...
        columns: [
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    if (enableDelete) {
                        bDel = `<button title='Apagar Notificação' class='notification-delete btn btn-outline-secondary' type='button' pk='${row.pk}'><span class='fas fa-trash-alt icon-red'></span></button>`;
                    }
...
function getReadEditButton(pk, state, small=false) {
    ...
    if (state) {
        bReadText = 'Marcar como não lido';
        bReadIcon = 'fas fa-bookmark icon-red';
        bReadTarget = 'unread';
    } else {
        bReadText = 'Marcar como lido';
        ...
        },
        {
            success: function(response) {
                if (response.length == 0) {
                    html = `<p class='text-muted'><em>Nenhuma notificação pendente</em><span class='fas fa-check-circle icon-green float-right'></span></p>`;
                } else {
                    // build up items
...
function closeNotificationPanel() {
    $('#notification-center').html(`<p class='text-muted'>Notificações irão carregar aqui</p>`);
}
