...
def update_attachments(apps, schema_editor):
    ...
    for app, model, target_model, model_ref in get_legacy_models():
        ...
        for attachment in LegacyAttachmentModel.objects.all():
            ...
            if attachment.attachment and default_storage.exists(
        ...
        if len(to_create) > 0:
            print(
                f"Migrating {len(to_create)} attachments for the legacy '{model}' model."
            )
    ...
    assert Attachment.objects.count() == N
...
def reverse_attachments(apps, schema_editor):
    ...
    for app, model, target_model, model_ref in get_legacy_models():
        ...
        if len(to_create) > 0:
            print(
                f"Reversing {len(to_create)} attachments for the legacy '{model}' model."
            )
