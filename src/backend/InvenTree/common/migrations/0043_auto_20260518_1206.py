...
def update_image_attachments(apps, schema_editor):
    ...
    attachments = Attachment.objects.exclude(attachment__isnull=True).exclude(
        attachment=''
    )
    ...
    progress = tqdm(
        total=attachments.count(), desc='Migration common.0043: Updating attachments'
    )
    ...
    for attachment in attachments:
        ...
        if not attachment.attachment.name or not default_storage.exists(
...
class Migration(migrations.Migration):
    dependencies = [('common', '0042_attachment_is_image_attachment_thumbnail')]

    operations = [
        migrations.RunPython(
            update_image_attachments, reverse_code=migrations.RunPython.noop
        )
    ]
