...
class InvenTreePlugin(VersionMixin, MixinBase, MetaBase):
    ...
    def hashed_file_lookup(self, *args) -> str | None:
        ...
        manifest_file = os.path.join(*self.get_static_path(), '.vite', 'manifest.json')
    ...
    def plugin_static_file(
        ...
        full_path = os.path.join(*self.get_static_path(), *file_path, file_name)
        ...
        url = os.path.join(settings.STATIC_URL, full_path)
