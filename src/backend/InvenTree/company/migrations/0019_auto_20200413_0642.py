...
def reverse_association(apps, schema_editor):  # pragma: no cover
    ...
    for index, row in enumerate(supplier_parts):
        ...
        response = cursor.execute(
            f'SELECT manufacturer_id FROM part_supplierpart WHERE id={supplier_part_id};'
        )
        ...
        response = cursor.execute(
            f'SELECT name from company_company where id={manufacturer_id};'
        )
        ...
        if row:
            ...
            print(f" - Manufacturer name: '{name}'")

            response = cursor.execute(
                f"UPDATE part_supplierpart SET manufacturer_name='{name}' WHERE id={supplier_part_id};"
            )
...
def associate_manufacturers(apps, schema_editor):
    ...
    def get_manufacturer_name(part_id):
        ...
        """
        query = f'SELECT manufacturer_name from part_supplierpart where id={part_id};'
    ...
    def link_part(part_id, name):
        ...
        if name in companies:  # pragma: no cover
            print(f" - Part[{part_id}]: '{name}' maps to existing manufacturer")
        ...
        if name in links:  # pragma: no cover
            print(f" - Part[{part_id}]: Mapped '{name}' - manufacturer <{links[name]}>")
    ...
    def create_manufacturer(part_id, input_name, company_name):
        ...
        manufacturer = Company.objects.create(
            name=company_name, description=company_name, is_manufacturer=True
        )
        ...
        print(f" - Part[{part_id}]: Created new manufacturer: '{company_name}'")

        # Update SupplierPart object in the database
        cursor.execute(
            f'update part_supplierpart set manufacturer_id={manufacturer.pk} where id={part_id};'
        )
    ...
    def map_part_to_manufacturer(part_id, idx, total):
        ...
        if not name or len(name) == 0:  # pragma: no cover
            print(f' - Part[{part_id}]: No manufacturer_name provided, skipping')
            return
        ...
        print(f'Checking part [{part_id}] ({idx + 1} of {total})')
        ...
        if not TESTING:  # pragma: no cover
            ...
            print(f"0) - Create new manufacturer '{name}'")
            print('')

            for i, m in enumerate(matches[:10]):
                print(f"{i + 1}) - Use manufacturer '{m}'")
            ...
            try:
                ...
                # Options 1) - n) select an existing manufacturer
                    ...
                    if n < len(matches):
                        ...
                        cursor.execute(
                            f'update company_company set is_manufacturer=true where id={company_id};'
                        )

                        # Link the company to the part
                        cursor.execute(
                            f'update part_supplierpart set manufacturer_id={company_id} where id={part_id};'
                        )
                        ...
                        print(
                            f" - Part[{part_id}]: Linked '{name}' to manufacturer '{company_name}'"
                        )
                ...
                elif response in companies:
                    link_part(part_id, companies[response])
                    return

                elif response in links:
    ...
    cursor = connection.cursor()
    response = cursor.execute(
        'select id, "MPN", "SKU", manufacturer_id, manufacturer_name from part_supplierpart;'
    )
    results = cursor.fetchall()
    ...
    for index, row in enumerate(results):
        ...
        if manufacturer_id is not None:  # pragma: no cover
            print(
                f' - SupplierPart <{pk}> already has a manufacturer associated (skipping)'
            )
            continue
