...
def reverse_currencies(apps, schema_editor):  # pragma: no cover
    ...
    response = cursor.execute(
        'SELECT id, price, price_currency from part_supplierpricebreak;'
    )
    ...
    for index, row in enumerate(results):
        ...
        response = cursor.execute(
            f'UPDATE part_supplierpricebreak set cost={price} where id={pk};'
        )
    ...
    for suffix in code_map:
        ...
        response = cursor.execute(
            f"UPDATE part_supplierpricebreak set currency_id={pk} where price_currency='{suffix}';"
        )
