...
def extract_purchase_price(apps, schema_editor):
    ...
    if items.count() > 0:  # pragma: no cover
        print(
            f'Found {items.count()} stock items with missing purchase price information'
        )
        ...
        if lines.exists():
            for line in lines:
                if getattr(line, 'purchase_price', None) is not None:
                    ...
                    print(
                        f'- Updating supplier price for {item.part.name} - {item.purchase_price} {item.purchase_price_currency}'
                    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            extract_purchase_price, reverse_code=migrations.RunPython.noop
        )
    ]
