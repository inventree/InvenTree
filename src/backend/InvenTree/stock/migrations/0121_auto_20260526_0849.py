...
def set_creation_date(apps, schema_editor):
    ...
    if creation_entries.count() > 0:
        progress = tqdm(
            total=creation_entries.count(),
            desc='stock.0121: Setting creation_date for StockItem entries',
        )
    ...
    if items.count() > 0:
        progress = tqdm(
            total=items.count(),
            desc='stock.0121: Setting creation_date for remaining StockItem entries',
        )

        for item in items:
            # Source the creation_date from the stock item
            earliest_entry = (
                StockItemTracking.objects.filter(item_id=item.pk)
                .order_by('date')
                .first()
            )
            ...
            raw_options = [
                item.updated,
                datetime.datetime(
                    item.stocktake_date.year,
                    item.stocktake_date.month,
                    item.stocktake_date.day,
                    tzinfo=utc,
                )
...
class Migration(migrations.Migration):
    ...
    dependencies = [('stock', '0120_stockitem_creation_date')]
