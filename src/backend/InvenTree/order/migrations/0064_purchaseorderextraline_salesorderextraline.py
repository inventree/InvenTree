...
def _convert_model(apps, line_item_ref, extra_line_ref, price_ref):
    ...
    print(
        f'Starting to convert - currently at {OrderExtraLine.objects.all().count()} {extra_line_ref} / {OrderLineItem.objects.all().count()} {line_item_ref} instance(s)'
    )
    for lineItem in items_to_change:
        ...
        lineItem.delete()
    print(
        f'Done converting line items - now at {OrderExtraLine.objects.all().count()} {extra_line_ref} / {OrderLineItem.objects.all().count()} {line_item_ref} instance(s)'
    )
...
def _reconvert_model(apps, line_item_ref, extra_line_ref):  # pragma: no cover
    ...
    for extra_line in OrderExtraLine.objects.all():
        ...
        if extra_line.context:
            context_string = extra_line.context.migration
...
def convert_line_items(apps, schema_editor):
    """Convert line items"""
    _convert_model(
        apps, 'PurchaseOrderLineItem', 'PurchaseOrderExtraLine', 'purchase_price'
    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.CreateModel(
            ...
            options={'abstract': False, 'verbose_name': 'Sales Order Extra Line'},
        ),
        migrations.CreateModel(
            ...
            options={'abstract': False, 'verbose_name': 'Purchase Order Extra Line'},
        ),
