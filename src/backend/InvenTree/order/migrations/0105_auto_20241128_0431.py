...
def update_shipment_date(apps, schema_editor):
    ...
    for order in orders:
        ...
        assert order.shipment_date is None, (
            f'SalesOrder {order.pk} has non-null shipment_date'
        )
        ...
        assert order.shipment_date is not None, (
            f'SalesOrder {order.pk} still has missing shipment_date'
        )
...
class Migration(migrations.Migration):
    dependencies = [('order', '0104_alter_returnorderlineitem_quantity')]

    operations = [
        migrations.RunPython(
            update_shipment_date, reverse_code=migrations.RunPython.noop
        )
    ]
