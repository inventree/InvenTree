...
def convert_legacy_labels(table_name, model_name, template_model):
    ...
    for row in rows:
        data = {fields[idx]: row[idx] for idx in range(len(fields))}
        ...
        if template_model.objects.filter(
            name=data['name'], model_type=model_name
        ).exists():
            print(
                f'LabelTemplate {data["name"]} already exists for {model_name} - skipping'
            )
            continue
...
class Migration(migrations.Migration):
    ...
    dependencies = [('report', '0025_labeltemplate')]
