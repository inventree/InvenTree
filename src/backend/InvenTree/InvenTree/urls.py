...
def get_backend_patterns():
    """Generate the backend URL patterns."""
    ...
    return [
        ...
        path('api/', include(get_api_patterns())),
        path(
            'api-doc/', SpectacularRedocView.as_view(url_name='schema'), name='api-doc'
        ),
        # Emails
...
def get_url_patterns():
    """Generate the URL patterns for the InvenTree application."""
    import plugin.urls
    import web.urls
    ...
    # Add in the API and backend URL patterns
    urlpatterns += get_backend_patterns()

    # Add in the platform URLs
    urlpatterns += web.urls.urlpatterns

    # Append custom plugin URLs (if custom plugin support is enabled)
    if settings.PLUGINS_ENABLED:
        urlpatterns.append(plugin.urls.get_plugin_urls())
    ...
    if settings.FRONTEND_SETTINGS.get('url_compatibility'):
        urlpatterns += web.urls.cui_compatibility_urls(settings.FRONTEND_URL_BASE)
...
# Expose the URL patterns for the InvenTree application
