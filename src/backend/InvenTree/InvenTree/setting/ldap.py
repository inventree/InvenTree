...
def get_ldap_config(debug: bool = False) -> dict:
    ...
    group_search_dn = get_setting('INVENTREE_LDAP_GROUP_SEARCH', 'ldap.group_search')

    find_group_perms = get_boolean_setting(
        'INVENTREE_LDAP_FIND_GROUP_PERMS', 'ldap.find_group_perms', True
    )

    # If group search DN is not configured (None or empty string),
    # group-based features cannot work.  Disable them gracefully with a
    # warning instead of letting django-auth-ldap crash at runtime with
    # a TypeError (see #12225).
    if not group_search_dn:
        if (
            find_group_perms
            or get_setting('INVENTREE_LDAP_MIRROR_GROUPS', 'ldap.mirror_groups')
            or get_setting('INVENTREE_LDAP_REQUIRE_GROUP', 'ldap.require_group')
            or get_setting('INVENTREE_LDAP_DENY_GROUP', 'ldap.deny_group')
        ):
            print(
    ...
    ldap_config = {
        ...
        'AUTH_LDAP_GROUP_SEARCH': django_auth_ldap.config.LDAPSearch(
            group_search_dn, ldap.SCOPE_SUBTREE, f'(objectClass={group_object_class})'
        )
        if group_search_dn
        else None,
